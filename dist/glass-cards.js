...
`;let wt=class extends ct{setConfig(t){this._config={title:"Home",entities:[],...t}}getCardSize(){return 2}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{title:"Home",entities:[]}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||this._config.entities.some(t=>e.states[t]!==this.hass.states[t])}_statusOf(t){const e=this.hass?.states[t];if(!e)return{label:"Unknown",home:!1,accent:"var(--g-dim)"};if("home"===e.state)return{label:"Home",home:!0,accent:"var(--g-green)"};if("not_home"===e.state)return{label:"Away",home:!1,accent:"var(--g-dim)"};return{label:e.state.charAt(0).toUpperCase()+e.state.slice(1),home:!1,accent:"var(--g-amber)"}}_relTime(t){if(!t)return"";const e=Math.round((Date.now()-new Date(t).getTime())/6e4);if(e<1)return"just now";if(e<60)return`${e}m ago`;const s=Math.round(e/60);return s<24?`${s}h ago`:`${Math.round(s/24)}d ago`}_open(t){_t(this,"hass-more-info",{entityId:t})}render(){if(!this._config||!this.hass)return Y;if(!this._config.entities.length)return yt("Add person entities","group");const t=this._config.entities.map(t=>({id:t,st:this.hass.states[t]})),e=t.filter(t=>"home"===t.st?.state).length;return G`
      <div class="card">
        ...
        <div class="people">
          ${t.map(t=>{if(!t.st)return G`<div class="missing">${t.id} not found</div>`;const e=this._statusOf(t.id),s=t.st.attributes.entity_picture,i=t.st.attributes.friendly_name??t.id,a=i.split(" ").map(t=>t[0]).slice(0,2).join("").toUpperCase();return G`
              <button class="person" @click=${()=>this._open(t.id)} title=${i}>
                <div class="avatar" style="--accent:${e.accent}">
                  ${s?G`<img src=${s} alt=${i} />`:G`<span class="mono-av">${a}</span>`}
                  ${e.home?G`<span class="ring"></span>`:Y}
                </div>
                <div class="who">
        ...
        <div class="top">
          <div class="cond">
            ...
            <div>
              ...
              ${n?G`<div class="wind">Wind ${n}</div>`:Y}
            </div>
          </div>
    ...
    `}};Ct.styles=[$t,o`
      ...
      .missing { text-align: center; color: var(--g-red-text); font-size: 13px; padding: 8px; }
    `],e([mt({attribute:!1})],Ct.prototype,"hass",void 0),e([ft()],Ct.prototype,"_config",void 0),Ct=e([pt("glass-weather-card")],Ct);let zt=class extends ct{setConfig(t){this._config={icon:"videocam",...t}}getCardSize(){return 3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{entity:"camera.garage",icon:"garage"}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||e.states[this._config.entity]!==this.hass.states[this._config.entity]}_stamp(t){const e=t?new Date(t):new Date,s=t=>String(t).padStart(2,"0");return`${e.getFullYear()}-${s(e.getMonth()+1)}-${s(e.getDate())} · ${s(e.getHours())}:${s(e.getMinutes())}:${s(e.getSeconds())}`}render(){if(!this._config||!this.hass)return Y;const t=this._config.entity?this.hass.states[this._config.entity]:void 0;if(!t)return yt("Select a camera entity","videocam");const e=t.attributes.entity_picture,s=this._config.name??t.attributes.friendly_name??this._config.entity,i="streaming"===t.state||"recording"===t.state||"idle"===t.state;return G`
      <div class="cam" @click=${()=>_t(this,"hass-more-info",{entityId:this._config.entity})}>
        ${e?G`<img src=${e} alt=${s} />`:Y}
        <div class="scan"></div>
        <div class="ts">${this._stamp(t.last_changed)}</div>
        ${i?G`<div class="live-badge"><span class="rec"></span>LIVE</div>`:G`<div class="live-badge off">OFFLINE</div>`}
        ${e?Y:G`<div class="ph">${xt("videocam",52)}</div>`}
    ...
    `}_renderShield(){const t=this._look(this._st.state),e=this._config.name??this._st.attributes.friendly_name??"Security";return G`<div class="card" style="border-color:${t.border}">
      ${this._header(t,e)}
      <div class="actions">${this._actionButtons()}</div>
    </div>`}_renderRadial(){const t=this._look(this._st.state),e=this._config.name??this._st.attributes.friendly_name??"Security";return G`<div class="card center" style="border-color:${t.border}">
      ...
      <div class="actions">${this._actionButtons()}</div>
    </div>`}_renderBar(){const t=this._look(this._st.state),e=this._config.name??this._st.attributes.friendly_name??"Security",s=this._st.state.startsWith("armed")||"triggered"===this._st.state;return G`<div class="bar" style="border-color:${t.border}">
    ...
    </div>`}_renderKeypad(){const t=this._config.code_length??4,e=Array.from({length:t},(t,e)=>e<this._code.length?"●":"○").join(" "),s=this._look(this._st.state);return G`<div class="card">
      <div class="disp"><span class="ms" style="font-size:22px;color:${s.accent}">${s.glyph}</span><div class="dots">${e}</div></div>
      <div class="pad">
        ${["1","2","3","4","5","6","7","8","9","back","0","ok"].map(t=>"back"===t?G`<button class="k" @click=${()=>this._key("back")}>${xt("backspace",20,"var(--g-dim)")}</button>`:"ok"===t?G`<button class="k ok" @click=${()=>this._key("ok")}>${xt("check",20,"var(--g-amber-ink)")}</button>`:G`<button class="k" @click=${()=>this._key(t)}>${t}</button>`)}
      </div>
    </div>`}_renderTriggered(){if("triggered"===this._st.state)return G`<div class="alert red">
      ...
      </div>`;const t=this._look(this._st.state);return G`<div class="alert" style="border-color:${t.border}">
      ...
      .missing { text-align: center; color: var(--g-red-text); font-size: 13px; padding: 8px; }
    `],e([mt({attribute:!1})],Nt.prototype,"hass",void 0),e([ft()],Nt.prototype,"_config",void 0),e([ft()],Nt.prototype,"_code",void 0),Nt=e([pt("glass-alarm-card")],Nt);const Mt=[["#7dd68a","var(--g-green)"],["#e0b24a","var(--g-amber)"],["#a97ff0","var(--g-purple)"],["#5ab9bd","var(--g-cyan)"]];let Pt=class extends ct{constructor(){super(...arguments),this._bars=[]}setConfig(t){this._config={title:"Energy",variant:"flow",...t},this._fetchedFor=void 0}getCardSize(){return"flow"===this._config?.variant||"production"===this._config?.variant?4:3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{title:"Energy",variant:"flow",solar:"sensor.solar_power",grid:"sensor.grid_power",battery:"sensor.battery_power"}}shouldUpdate(t){if(t.has("_config")||t.has("_bars"))return!0;const e=t.get("hass");if(!e||!this.hass||!this._config)return!0;const s=this._config;return[s.solar,s.grid,s.battery,s.battery_soc,s.house,s.today,s.imported,s.exported,s.saved,s.production,...s.meters??[]].filter(Boolean).some(t=>e.states[t]!==this.hass.states[t])}updated(){if(!this.hass||!this._config||"production"!==this._config.variant||!this._config.production)return;const t=this.hass.states[this._config.production];t&&t.last_updated!==this._fetchedFor&&(this._fetchedFor=t.last_updated,this._loadProduction(this._config.production))}async _loadProduction(t){const e=new Date;e.setHours(0,0,0,0);try{const s=(await this.hass.callWS({type:"recorder/statistics_during_period",start_time:e.toISOString(),statistic_ids:[t],period:"hour"}))[t]??[];this._bars=s.map(t=>({v:Math.max(0,Number(t.change??t.state??0)),label:String(new Date(Number(t.start)).getHours())}))}catch{this._bars=[]}}_watts(t){if(!t||!this.hass)return null;const e=this.hass.states[t];if(!e||"unavailable"===e.state||"unknown"===e.state)return null;const s=Number(e.state);if(Number.isNaN(s))return null;return"kw"===String(e.attributes.unit_of_measurement??"").toLowerCase()?1e3*s:s}_kw(t){return(t/1e3).toFixed(1)}_stateNum(t){const e=t?Number(this.hass.states[t]?.state):NaN;return Number.isNaN(e)?null:e}_label(t){return this.hass.states[t]?.attributes.friendly_name??t}render(){if(!this._config||!this.hass)return Y;switch(this._config.variant){case"ring":return this._renderRing();case"stats":return this._renderStats();case"meters":return this._renderMeters();case"production":return this._renderProduction();default:return this._renderFlow()}}_head(t){return G`<div class="hdr"><div class="hdr-l">${xt("bolt",22,"var(--g-amber)")}<span class="title">${this._config.title}</span></div>${t??Y}</div>`}_renderFlow(){const t=this._watts(this._config.solar),e=this._watts(this._config.grid),s=this._watts(this._config.battery);let i=this._watts(this._config.house);null==i&&(i=(t??0)+(e??0)+(s??0));const a=this._stateNum(this._config.battery_soc),r=(t??0)>20,n=(e??0)>20,o=(e??0)<-20,l=(s??0)>20,d=(s??0)<-20,c=o?{label:"Exporting",purple:!1}:n?{label:"Grid import",purple:!0}:{label:"Self-powered",purple:!1},g=(t,e,s=!1)=>t?`stroke:${e};animation:g-flow 0.7s linear infinite${s?" reverse":""};`:"stroke:rgba(255,255,255,0.06);";return G`<div class="card">
      ${this._head(G`<div class="badge ${c.purple?"":"green"}" style=${c.purple?"background:rgba(201,166,255,0.14);color:var(--g-purple)":""}><span class="dot pulse"></span>${c.label}</div>`)}
      <div class="flow">
        <svg viewBox="0 0 408 264" preserveAspectRatio="xMidYMid meet">
          ${F`
            ...
            <path d="M242 148 L316 176" fill="none" stroke-width="3.5" stroke-linecap="round" stroke-dasharray="2 12" style=${g(l||d,"var(--g-green)",d)}></path>
          `}
        </svg>
        ${null!=t?G`<div class="node solar"><div class="chip amber">${xt("solar_power",20,"var(--g-amber)")}<span>${this._kw(t)}</span></div><span class="cap">Solar · kW</span></div>`:Y}
        <div class="node home">${xt("home",26,"var(--g-amber)")}<span class="watts t-num">${Math.round(i)}</span><span class="cap">Watts now</span></div>
        ${null!=e?G`<div class="node grid"><div class="chip">${xt("bolt",20,"var(--g-purple)")}<span style="color:var(--g-purple)">${this._kw(Math.abs(e))}</span></div><span class="cap">Grid · kW${n?" (import)":o?" (export)":" (idle)"}</span></div>`:Y}
        ${null!=s?G`<div class="node batt"><div class="chip green">${xt(d?"battery_charging_full":"battery_full",20,"var(--g-green)")}<span style="color:var(--g-green)">${this._kw(Math.abs(s))}</span></div><span class="cap">Battery${null!=a?` · ${Math.round(a)}%`:" · kW"}</span></div>`:Y}
      </div>
    </div>`}_renderRing(){const t=this._config,e=this._stateNum(t.battery_soc)??0,s=this._watts(t.solar);let i=this._watts(t.house);null==i&&(i=(s??0)+(this._watts(t.grid)??0)+(this._watts(t.battery)??0));const a=this._watts(t.battery),r=null!=a&&Math.abs(a)>5?G`<div class="delta" style="color:${a<0?"var(--g-green)":"var(--g-amber)"}">${a<0?"↓":"↑"} ${Math.abs(Math.round(a))} W</div>`:Y;return G`<div class="card">
      ${this._head()}
      <div class="ring-row">
      ...
      ${t.today||t.grid?G`<div class="ring-tiles">
            ${t.today?G`<div class="stat"><div class="t-num sv">${this._fmt(t.today).val} <span class="su">${this._fmt(t.today).unit||"kWh"}</span></div><div class="sc">Solar today</div></div>`:Y}
            ${t.grid?G`<div class="stat"><div class="t-num sv">${(this._watts(t.grid)??0)>5?"Import":(this._watts(t.grid)??0)<-5?"Export":"On"} <span style="color:var(--g-green)">●</span></div><div class="sc">Grid status</div></div>`:Y}
          </div>`:Y}
      ...
      ${s.length?G`<div class="tri">
            ${s.map(t=>{const e=this._fmt(t.id);return G`<div class="stat"><div class="t-num sv" style="color:${t.color}">${e.val}${e.unit?G` <span class="su">${e.unit}</span>`:Y}</div><div class="sc">${this._label(t.id)}</div></div>`})}
          </div>`:Y}
    </div>`}_renderMeters(){const t=this._config,e=[...t.battery_soc?[t.battery_soc]:[],...t.meters??[]];return G`<div class="card">
      ${this._head()}
      <div class="meters">
        ${e.map((e,s)=>{const i=Math.max(0,Math.min(100,this._stateNum(e)??0)),[a,r]=Mt[s%Mt.length];return G`<div class="m">
    ...
    </div>`}_renderProduction(){const t=this._bars.reduce((t,e)=>t+e.v,0),e=Math.max(.001,...this._bars.map(t=>t.v));return G`<div class="card">
      ${this._head(G`<div class="t-num total">${t.toFixed(1)} <span class="su">kWh</span></div>`)}
      ${this._bars.length?G`<div class="bars">
            ${this._bars.map(t=>G`<div class="bar-col"><div class="bar" style="height:${t.v/e*100}%;background:${t.v>=.66*e?"var(--g-amber)":"var(--g-amber-deep)"}"></div><span class="bl">${t.label}</span></div>`)}
          </div>`:G`<div class="empty">No production data for today yet.</div>`}
    </div>`}};Pt.styles=[$t,o`
    ...
    `],e([mt({attribute:!1})],Tt.prototype,"hass",void 0),e([ft()],Tt.prototype,"_config",void 0),Tt=e([pt("glass-tile-card")],Tt);let Lt=class extends ct{constructor(){super(...arguments),this._rows=[]}setConfig(t){this._rows=(t.entities??[]).map(t=>"string"==typeof t?{entity:t}:t),this._config={title:"Sensors",...t}}getCardSize(){return this._rows.length+1}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{title:"Sensors",entities:[]}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!e||!this.hass||this._rows.some(t=>e.states[t.entity]!==this.hass.states[t.entity])}_resolveType(t){if(t.type&&"auto"!==t.type)return t.type;const e=this.hass.states[t.entity],s=e?.attributes.device_class;return t.entity.startsWith("cover.")||"garage"===s?"garage":"window"===s?"window":"door"}_isOpen(t){const e=this.hass.states[t];return!!e&&("open"===e.state||"on"===e.state||"opening"===e.state)}_visual(t,e){return"garage"===t?G`<div class="v garage">
        <div class="slats" style="transform:translateY(${e?"-100%":"0"})">
          <span></span><span></span><span></span><span></span>
        </div>
        <div class="floor"></div>
      </div>`:"window"===t?G`<div class="v window">
        <div class="pane top" style="transform:translateY(${e?"-60%":"0"})"></div>
        <div class="pane bot"></div>
      </div>`:G`<div class="v door">
      <div class="frame"><div class="leaf" style="transform:rotateY(${e?"-62deg":"0deg"})"><span class="knob"></span></div></div>
    </div>`}render(){if(!this._config||!this.hass)return Y;if(!this._rows.length)return yt("Add door / window / cover sensors","sensors");const t=this._rows.filter(t=>this._isOpen(t.entity)).length;return G`
      <div class="card">
        <div class="hdr">
        ...
        ${this._rows.map((t,e)=>{const s=this.hass.states[t.entity];if(!s)return G`<div class="line"><div class="missing">${t.entity} not found</div></div>`;const i=this._resolveType(t),a=this._isOpen(t.entity),r=t.name??s.attributes.friendly_name??t.entity;return G`
    ...
    `}};Rt.styles=[$t,o`
      ...
      .missing { text-align: center; color: var(--g-red-text); font-size: 13px; padding: 8px; }
    `],e([mt({attribute:!1})],Rt.prototype,"hass",void 0),e([ft()],Rt.prototype,"_config",void 0),Rt=e([pt("glass-media-card")],Rt);let Ht=class extends ct{setConfig(t){this._config={title:"Overhead",max:4,...t}}getCardSize(){return 2}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{entity:"sensor.flightradar24_current_in_area",title:"Overhead"}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||e.states[this._config.entity]!==this.hass.states[this._config.entity]}_route(t){const e=t.airport_origin_code_iata||t.airport_origin_city,s=t.airport_destination_code_iata||t.airport_destination_city;return e&&s?`${e} → ${s}`:e||s||""}render(){if(!this._config||!this.hass)return Y;const t=this._config.entity?this.hass.states[this._config.entity]:void 0;if(!t)return yt("Select a Flightradar24 sensor","flight");const e=t.attributes.flights??[],s=e.length;return G`
      <div class="card" @click=${()=>_t(this,"hass-more-info",{entityId:this._config.entity})}>
        <div class="hdr">
        ...
        ${0===s?G`<div class="empty">${xt("travel_explore",30,"var(--g-faint)")}<span>No aircraft in your area</span></div>`:e.slice(0,this._config.max).map(t=>{const e=t.callsign||t.flight_number||"Unknown",s=[null!=t.altitude?`${Math.round(Number(t.altitude)).toLocaleString()} ft`:null,null!=t.ground_speed?`${Math.round(Number(t.ground_speed))} kt`:null,t.aircraft_code||t.aircraft_model].filter(Boolean);return G`
                <div class="row">
                  <div class="plane" style=${null!=t.heading?`transform:rotate(${Number(t.heading)}deg)`:""}>
                    ${xt("flight",20,"var(--g-cyan)")}
                  </div>
                  <div class="info">
                    <div class="cs">${e}${this._route(t)?G`<span class="rt">${this._route(t)}</span>`:Y}</div>
                    ${s.length?G`<div class="det">${s.join(" · ")}</div>`:Y}
                  </div>
                  ${null!=t.distance?G`<span class="dist">${Number(t.distance).toFixed(1)} km</span>`:Y}
                </div>
              `})}
      </div>
    `}};Ht.styles=[$t,o`
      ...
      <div class="card">
        ...
        ${a?G`
              <div class="active">
                ...
                <div class="stat-rows">
                  ${r.map(t=>G`<div class="srow"><div class="t-num sval">${this._statValue(t)}</div><div class="scap">${this._statLabel(t)}</div></div>`)}
                </div>
              </div>
            `:r.length?G`<div class="stat-grid">
                ${r.map(t=>G`<div class="stile"><div class="t-num sval">${this._statValue(t)}</div><div class="scap">${this._statLabel(t)}</div></div>`)}
              </div>`:Y}

        ${t.toggle?G`<button class="power ${n?"on":""}" @click=${this._toggle}>
    ...
    `],e([mt({attribute:!1})],Dt.prototype,"hass",void 0),e([ft()],Dt.prototype,"_config",void 0),Dt=e([pt("glass-appliance-card")],Dt);let It=class extends ct{constructor(){super(...arguments),this._toggles=[]}setConfig(t){this._toggles=(t.toggles??[]).map(t=>"string"==typeof t?{entity:t}:t),this._config={name:"Refrigerator",subtitle:"Kitchen",...t}}getCardSize(){return 3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{name:"Refrigerator",subtitle:"Kitchen",door:"",fridge_temp:"",freezer_temp:""}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");if(!e||!this.hass||!this._config)return!0;const s=[this._config.door,this._config.wifi,this._config.fridge_temp,this._config.freezer_temp,...this._toggles.map(t=>t.entity)].filter(Boolean);return s.some(t=>e.states[t]!==this.hass.states[t])}_num(t){const e=t?this.hass.states[t]:void 0;if(!e)return null;const s=Number(e.state);return{value:Number.isNaN(s)?null:s,min:Number(e.attributes.min??-30),max:Number(e.attributes.max??30),step:Number(e.attributes.step??1)}}_step(t,e,s){s.stopPropagation();const i=this._num(t);if(!t||!i||null==i.value)return;const a=Math.min(i.max,Math.max(i.min,i.value+e*i.step));this.hass.callService("number","set_value",{entity_id:t,value:a})}_toggle(t,e){e.stopPropagation(),this.hass.callService("homeassistant","toggle",{entity_id:t})}_tempTile(t,e,s,i){const a=this._num(i);return G`
      <div class="tt">
        ...
        ${a?G`<div class="range">range ${a.min}…${a.max} °C</div>`:Y}
      </div>
    `}render(){if(!this._config||!this.hass)return Y;const t=this._config,e=!!t.door&&"on"===this.hass.states[t.door]?.state,s=t.wifi?"on"===this.hass.states[t.wifi]?.state:null;return G`
      <div class="card">
        <div class="head">
          ...
          <div class="who">
            ...
            <div class="sub">
              ${t.subtitle}${null!=s?G` · ${xt("wifi",14,s?"var(--g-green)":"var(--g-dim)")}${s?"Connected":"Offline"}`:Y}
            </div>
          </div>
          ${t.door?G`<button class="pill ${e?"open":"closed"}" @click=${()=>_t(this,"hass-more-info",{entityId:t.door})}>
        ...
        ${this._toggles.length?G`<div class="rows">
              ${this._toggles.map(e=>{const s=this.hass.states[e.entity],i="on"===s?.state;let a=e.name??s?.attributes.friendly_name??e.entity;t.name&&a.toLowerCase().startsWith(t.name.toLowerCase()+" ")&&(a=a.slice(t.name.length+1));const r=e.icon??(/cool/i.test(a)?"mode_cool":"bolt");return G`
                  <div class="row">
    ...
    `],e([mt({attribute:!1})],It.prototype,"hass",void 0),e([ft()],It.prototype,"_config",void 0),It=e([pt("glass-fridge-card")],It);const qt={green:"var(--g-green)",amber:"var(--g-amber)",cyan:"var(--g-cyan)",purple:"var(--g-purple)",red:"var(--g-red-text)"};let Gt=class extends ct{setConfig(t){this._config={name:"Pool Pump",subtitle:"Backyard",...t}}getCardSize(){return 3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{name:"Pool Pump",subtitle:"Backyard",switch:"",energy:""}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||[this._config.switch,this._config.energy].filter(Boolean).some(t=>e.states[t]!==this.hass.states[t])}_toggle(t){t.stopPropagation(),this._config.switch&&this.hass.callService("homeassistant","toggle",{entity_id:this._config.switch})}render(){if(!this._config||!this.hass)return Y;const t=this._config,e=t.switch?this.hass.states[t.switch]:void 0,s=!e||"unavailable"===e.state,i="on"===e?.state,a=qt[t.color??"green"]??"var(--g-green)",r=s?"var(--g-dim)":i?a:"var(--g-amber)",n=s?"Unavailable":i?"Running":"Idle",o=t.energy?this.hass.states[t.energy]:void 0,l=o&&!Number.isNaN(Number(o.state))?Number(o.state).toFixed(1):"—",d=o?.attributes.unit_of_measurement??"kWh",c=i?"animation:g-spin 1.1s linear infinite;transform-origin:48px 48px;":"",g=i?"animation:g-spin 6s linear infinite;transform-origin:48px 48px;":"";return G`
      <div class="card" style="border-color:${i?`color-mix(in srgb, ${a} 30%, transparent)`:"var(--g-hair)"}">
        <div class="head">
          ...
          <div class="who">
            <div class="title">${t.name}${i?G` <span class="live">LIVE</span>`:Y}</div>
        ...
        <div class="body">
          <div class="impeller">
            <svg viewBox="0 0 96 96" width="96" height="96">
              ${F`
                ...
                <g fill="none" stroke="${r}" stroke-width="4.5" stroke-linecap="round" style="${c}">
          ...
          <div class="right">
            ...
            <div class="energy" @click=${()=>t.energy&&_t(this,"hass-more-info",{entityId:t.energy})}>
              ...
              <div>
                <div class="t-num ev">${l} <span class="eu">${d}</span></div>
    ...
    `],e([mt({attribute:!1})],Gt.prototype,"hass",void 0),e([ft()],Gt.prototype,"_config",void 0),Gt=e([pt("glass-pool-card")],Gt);const Ft={door:"door_front",window:"window",connectivity:"wifi",running:"check_circle"};let Vt=class extends ct{setConfig(t){this._config={name:"Dishwasher",subtitle:"Kitchen",level_max:4,...t}}getCardSize(){return 3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{name:"Dishwasher",subtitle:"Kitchen",status:""}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");if(!e||!this.hass||!this._config)return!0;const s=this._config;return[s.status,s.alert,...s.tiles??[],...s.levels??[]].filter(Boolean).some(t=>e.states[t]!==this.hass.states[t])}_label(t){let e=this.hass.states[t]?.attributes.friendly_name??t;const s=this._config.name;return s&&e.toLowerCase().startsWith(s.toLowerCase()+" ")&&(e=e.slice(s.length+1)),e}_pretty(t){return t.replace(/_/g," ").replace(/^\w/,t=>t.toUpperCase())}_statusLook(t){const e=t.toLowerCase(),s=this._pretty(t);return/(run|washing|active|start)/.test(e)?{color:"var(--g-cyan)",label:s}:/(complete|finish|clean|done)/.test(e)?{color:"var(--g-green)",label:s}:/(pause|delay)/.test(e)?{color:"var(--g-amber)",label:s}:/(off|idle|power_off|standby)/.test(e)?{color:"var(--g-dim)",label:"Power off"}:{color:"var(--g-dim)",label:s}}_tile(t){const e=this.hass.states[t];if(!e)return Y;const s=e.attributes.device_class,i=t.split(".")[0];let a,r=!1;"binary_sensor"===i||"switch"===i?(r="on"===e.state,a="door"===s||"window"===s||"opening"===s?r?"Open":"Closed":"connectivity"===s?r?"Online":"Offline":r?"On":"Off"):a=this._pretty(e.state);return G`<div class="tile" @click=${()=>_t(this,"hass-more-info",{entityId:t})}>
      ${xt(Ft[s??""]??"info",20,"door"===s&&!r||"connectivity"===s&&r?"var(--g-green)":"var(--g-dim)")}
      <div><div class="tv">${a}</div><div class="tl">${this._label(t)}</div></div>
    </div>`}_level(t){const e=this.hass.states[t];if(!e)return Y;const s=this._config.level_max??4,i=String(e.state).match(/(\d+(\.\d+)?)/),a=i?Number(i[1]):0,r=Math.max(0,Math.min(100,a/s*100));return G`<div class="lv">
      <div class="lv-h"><span>${this._label(t)}</span><span class="lv-n">${a} / ${s}</span></div>
      <div class="meter"><span style="width:${r}%"></span></div>
    </div>`}render(){if(!this._config||!this.hass)return Y;const t=this._config,e=t.status?this.hass.states[t.status]?.state:void 0,s=e?this._statusLook(e):null,i=!!t.alert&&"on"===this.hass.states[t.alert]?.state,a=(t.tiles??[]).filter(t=>this.hass.states[t]),r=(t.levels??[]).filter(t=>this.hass.states[t]);return G`
      <div class="card">
        <div class="head">
          ...
          ${s?G`<span class="pill" style="color:${s.color}"><span class="dot" style="background:${s.color}"></span>${s.label}</span>`:Y}
        </div>

        ${i?G`<div class="alert">${xt("opacity",22,"var(--g-amber)")}<div><div class="a-t">${t.alert_text??"Attention needed"}</div></div></div>`:Y}

        ${a.length?G`<div class="tiles">${a.slice(0,3).map(t=>this._tile(t))}</div>`:Y}
        ${r.length?G`<div class="levels">${r.map(t=>this._level(t))}</div>`:Y}
      </div>
    `}};Vt.styles=[$t,o`
    ...
    `],e([mt({attribute:!1})],Kt.prototype,"hass",void 0),e([ft()],Kt.prototype,"_config",void 0),Kt=e([pt("glass-stat-card")],Kt);let Xt=class extends ct{constructor(){super(...arguments),this._groups=[]}setConfig(t){t.groups?.length?this._groups=t.groups:t.entities?.length?this._groups=[{entities:t.entities}]:this._groups=[],this._config={title:"Lights",...t}}getCardSize(){return 1+this._groups.reduce((t,e)=>t+Math.ceil(e.entities.length/3),0)}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{title:"Lights",entities:[]}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!e||!this.hass||this._all().some(t=>e.states[t]!==this.hass.states[t])}_all(){return this._groups.flatMap(t=>t.entities)}_isOn(t){return"on"===this.hass.states[t]?.state}_toggle(t,e){e.stopPropagation(),this.hass.callService("homeassistant","toggle",{entity_id:t})}_master(t,e){e.stopPropagation();const s=t.entities.some(t=>this._isOn(t));this.hass.callService("light",s?"turn_off":"turn_on",{entity_id:t.entities})}_name(t){return this.hass.states[t]?.attributes.friendly_name??t}render(){if(!this._config||!this.hass)return Y;if(!this._all().length)return yt("Add lights","lightbulb");const t=this._all().filter(t=>this._isOn(t)).length;return G`
      <div class="card">
        ...
        ${this._groups.map(t=>{const e=t.entities.filter(t=>this._isOn(t)).length,s=e>0;return G`
            <div class="group">
              ${t.name?G`<div class="g-hdr">
              ...
              <div class="tiles">
                ${t.entities.map(t=>{const e=this._isOn(t),s=this.hass.states[t],i=e&&null!=s?.attributes.brightness?`${Math.round(Number(s.attributes.brightness)/255*100)}%`:e?"On":"Off";return G`
    ...
    `],e([mt({attribute:!1})],Xt.prototype,"hass",void 0),e([ft()],Xt.prototype,"_config",void 0),Xt=e([pt("glass-lights-card")],Xt);const Jt={scene:"palette",script:"play_arrow",automation:"bolt"};let Qt=class extends ct{constructor(){super(...arguments),this._scenes=[]}setConfig(t){this._scenes=(t.scenes??[]).map(t=>"string"==typeof t?{entity:t}:t),this._config={columns:4,...t}}getCardSize(){return Math.ceil(this._scenes.length/(this._config?.columns??4))+1}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{scenes:[]}}shouldUpdate(){return!0}_activate(t){const e=t.split(".")[0],s="scene"===e?"scene.turn_on":"script"===e?"script.turn_on":"automation"===e?"automation.trigger":"homeassistant.turn_on",[i,a]=s.split(".");this.hass.callService(i,a,{entity_id:t})}render(){return this._config&&this.hass?this._scenes.length?G`
      <div class="card">
        ${this._config.title?G`<div class="hdr"><div class="hdr-l">${xt("auto_awesome",20,"var(--g-amber)")}<span class="title">${this._config.title}</span></div></div>`:Y}
        <div class="grid" style="grid-template-columns:repeat(${this._config.columns}, 1fr)">
          ${this._scenes.map(t=>{const e=this.hass.states[t.entity],s=t.name??e?.attributes.friendly_name??t.entity,i=t.icon??Jt[t.entity.split(".")[0]]??"palette";return G`<button class="scene" @click=${()=>this._activate(t.entity)}>${xt(i,24,"var(--g-text-hi)")}<span class="sn">${s}</span></button>`})}
        </div>
      </div>
    `:yt("Add scenes or scripts","auto_awesome"):Y}};Qt.styles=[$t,o`
      .card { display: flex; flex-direction: column; gap: 14px; }
    ...
    `],e([mt({attribute:!1})],Qt.prototype,"hass",void 0),e([ft()],Qt.prototype,"_config",void 0),Qt=e([pt("glass-scenes-card")],Qt);const Zt=[[255,92,92],[243,208,106],[185,246,166],[135,221,225],[130,170,255],[201,166,255],[255,150,200],[255,244,224]];let te=class extends ct{setConfig(t){this._config=t}getCardSize(){return 4}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{entity:""}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||e.states[this._config.entity]!==this.hass.states[this._config.entity]}get _st(){return this.hass.states[this._config.entity]}_modes(){return this._st?.attributes.supported_color_modes??[]}_brightPct(){const t=this._st?.attributes.brightness;return null!=t?Math.round(Number(t)/255*100):0}_rgb(){const t=this._st?.attributes.rgb_color;return t&&3===t.length?[t[0],t[1],t[2]]:null}_clickPct(t){const e=t.currentTarget.getBoundingClientRect();return Math.max(0,Math.min(1,(t.clientX-e.left)/e.width))}_toggle(t){t.stopPropagation(),this.hass.callService("light","toggle",{entity_id:this._config.entity})}_setBright(t){const e=Math.max(1,Math.round(100*this._clickPct(t)));this.hass.callService("light","turn_on",{entity_id:this._config.entity,brightness_pct:e})}_setTemp(t){const e=Number(this._st.attributes.min_color_temp_kelvin??2e3),s=Number(this._st.attributes.max_color_temp_kelvin??6500),i=Math.round(e+this._clickPct(t)*(s-e));this.hass.callService("light","turn_on",{entity_id:this._config.entity,color_temp_kelvin:i})}_setRgb(t){this.hass.callService("light","turn_on",{entity_id:this._config.entity,rgb_color:t})}render(){if(!this._config||!this.hass)return Y;if(!this._st)return yt("Select a light","lightbulb");const t="on"===this._st.state,e=this._modes(),s=t&&e.some(t=>["brightness","color_temp","hs","rgb","rgbw","rgbww","xy","white"].includes(t)),i=t&&e.includes("color_temp"),a=t&&e.some(t=>["hs","rgb","rgbw","rgbww","xy"].includes(t)),r=this._rgb(),n=this._brightPct(),o=this._config.name??this._st.attributes.friendly_name??this._config.entity,l=t?r?`rgb(${r.join(",")})`:"var(--g-amber)":"var(--g-faint)",d=t?`radial-gradient(circle at 50% 45%, ${r?`rgba(${r.join(",")},0.35)`:"rgba(243,208,106,0.3)"} 0%, transparent 70%)`:"transparent";return G`
      <div class="card">
        ...
        <div class="glow-wrap">
          <div class="glow" style="background:${d}"></div>
        ...
        ${a?G`<div class="swatches">
              ${Zt.map(t=>G`<button class="sw" style="background:rgb(${t.join(",")})" @click=${()=>this._setRgb(t)}></button>`)}
            </div>`:Y}
      </div>
    `}};te.styles=[$t,o`
      .card { display: flex; flex-direction: column; gap: 18px; }
    ...
    `],e([mt({attribute:!1})],te.prototype,"hass",void 0),e([ft()],te.prototype,"_config",void 0),te=e([pt("glass-light-card")],te);const ee={off:{icon:"power_settings_new",color:"var(--g-dim)"},cool:{icon:"ac_unit",color:"var(--g-cyan)"},heat:{icon:"local_fire_department",color:"var(--g-amber)"},dry:{icon:"water_drop",color:"var(--g-purple)"},fan_only:{icon:"mode_fan",color:"var(--g-cyan)"},auto:{icon:"autorenew",color:"var(--g-green)"},heat_cool:{icon:"device_thermostat",color:"var(--g-green)"}};let se=class extends ct{setConfig(t){this._config=t}getCardSize(){return 3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{entity:""}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||e.states[this._config.entity]!==this.hass.states[this._config.entity]}get _st(){return this.hass.states[this._config.entity]}_setTemp(t,e){e.stopPropagation();const s=this._st.attributes,i=Number(s.target_temp_step??.5),a=Number(s.temperature);if(Number.isNaN(a))return;const r=Number(s.min_temp??7),n=Number(s.max_temp??35),o=Math.min(n,Math.max(r,a+t*i));this.hass.callService("climate","set_temperature",{entity_id:this._config.entity,temperature:o})}_setMode(t){this.hass.callService("climate","set_hvac_mode",{entity_id:this._config.entity,hvac_mode:t})}_setFan(t){this.hass.callService("climate","set_fan_mode",{entity_id:this._config.entity,fan_mode:t})}_toggle(t){t.stopPropagation();if("off"!==this._st.state)this._setMode("off");else{const t=this._st.attributes.hvac_modes??[];this._setMode(t.find(t=>"off"!==t)??"cool")}}render(){if(!this._config||!this.hass)return Y;if(!this._st)return yt("Select a climate entity","mode_fan");const t=this._st.attributes,e=this._st.state,s="off"!==e,i=ee[e]??ee.off,a=this._config.name??t.friendly_name??"Aircon",r=t.current_temperature,n=null!=t.temperature?Number(t.temperature):null,o=t.hvac_action??e,l=(t.hvac_modes??[]).filter(t=>"off"!==t),d=t.fan_modes??[];return G`
      <div class="card">
        ...
        ${l.length?G`<div class="seg">
              ${l.map(t=>G`<button class="sg ${e===t?"on":""}" title=${t} @click=${()=>this._setMode(t)}>${xt((ee[t]??{icon:"circle"}).icon,19)}</button>`)}
            </div>`:Y}

        ${d.length?G`<div class="fan-row">
              ${xt("air",18,"var(--g-dim)")}
              <div class="seg fan">
                ${d.map(e=>G`<button class="sg txt ${t.fan_mode===e?"on":""}" @click=${()=>this._setFan(e)}>${e}</button>`)}
              </div>
            </div>`:Y}
      </div>
    `}};se.styles=[$t,o`
      ...
      .missing { text-align: center; color: var(--g-red-text); font-size: 13px; padding: 8px; }
    `],e([mt({attribute:!1})],se.prototype,"hass",void 0),e([ft()],se.prototype,"_config",void 0),se=e([pt("glass-aircon-card")],se);let ie=class extends ct{constructor(){super(...arguments),this._modes=[]}setConfig(t){this._modes=(t.modes??[]).map(t=>"string"==typeof t?{entity:t}:t),this._config={name:"Geyser",subtitle:"Water heater",min_temp:20,max_temp:70,...t}}getCardSize(){return 3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{name:"Geyser",subtitle:"Bathroom",power:"",current:"",target:""}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");if(!e||!this.hass||!this._config)return!0;const s=this._config,i=[s.power,s.current,s.target,s.solar,s.power_sensor,s.solar_mode,...this._modes.map(t=>t.entity)].filter(Boolean);return i.some(t=>e.states[t]!==this.hass.states[t])}_num(t){const e=t?Number(this.hass.states[t]?.state):NaN;return Number.isNaN(e)?null:e}_isOn(t){if(!t)return!1;const e=this.hass.states[t];return!!e&&("on"===e.state||t.startsWith("water_heater.")&&"off"!==e.state)}_toggle(t,e){if(e.stopPropagation(),!t)return;const s=t.startsWith("water_heater.")?"water_heater":"homeassistant";this.hass.callService(s,"toggle",{entity_id:t})}_step(t,e){e.stopPropagation();const s=this._config.target;if(!s)return;const i=this.hass.states[s];if(!i)return;const a=s.split(".")[0];if("number"===a){const e=Number(i.state);if(Number.isNaN(e))return;const a=Number(i.attributes.min??this._config.min_temp??20),r=Number(i.attributes.max??this._config.max_temp??70),n=Number(i.attributes.step??1);this.hass.callService("number","set_value",{entity_id:s,value:Math.min(r,Math.max(a,e+t*n))})}else{const e=Number(i.attributes.temperature);if(Number.isNaN(e))return;const r=Number(i.attributes.target_temp_step??1);this.hass.callService(a,"set_temperature",{entity_id:s,temperature:e+t*r})}}_target(){const t=this._config.target;if(!t)return null;const e=this.hass.states[t];return e?t.startsWith("number.")?this._num(t):Number(e.attributes.temperature)||null:null}_fillColor(t){return`hsl(${Math.round(200-170*t)}, 72%, 52%)`}render(){if(!this._config||!this.hass)return Y;const t=this._config,e=this._isOn(t.power),s=this._num(t.current),i=this._target(),a=t.min_temp??20,r=t.max_temp??70,n=null!=s?Math.max(.05,Math.min(1,(s-a)/(r-a))):.5,o=e&&null!=s&&null!=i?s<i-.5:e,l=e?o?"#ff8c42":"var(--g-green)":"var(--g-dim)",d=e?o?"Heating":"Ready":"Off",c=o?"#ff8c42":"rgba(255,140,66,0.2)",g=this._isOn(t.solar_mode);return G`
      <div class="card">
        <div class="head">
          <div class="ibox" style="background:rgba(255,140,66,0.14)">${xt("water_heater",26,l)}</div>
          <div class="who"><div class="title">${t.name}</div><div class="sub">${t.subtitle}</div></div>
          ${t.power?G`<div class="toggle ${e?"on":""}" @click=${e=>this._toggle(t.power,e)}><div class="knob"></div></div>`:Y}
        </div>
        ...
        <div class="body">
          <div class="tank">
            ...
            <div class="element" style="background:${c};box-shadow:0 0 12px ${c}"></div>
            ${o?G`<span class="rise" style="left:24px"></span><span class="rise" style="left:44px;animation-delay:.5s"></span><span class="rise" style="left:62px;animation-delay:1s"></span>`:Y}
            <div class="cur">${null!=s?Math.round(s):"—"}°</div>
          </div>

          <div class="right">
            <span class="pill" style="color:${l}"><span class="dot" style="background:${l}"></span>${d}</span>
            <div class="stepper">
            ...
            ${t.solar||t.power_sensor?G`<div class="tiles">
                  ${t.solar?G`<div class="tile"><div class="th">${xt("wb_sunny",14,"var(--g-amber)")}Solar</div><div class="t-num tvv">${this._num(t.solar)??"—"}°</div></div>`:Y}
                  ${t.power_sensor?G`<div class="tile"><div class="th">${xt("bolt",14,"var(--g-purple)")}Power</div><div class="t-num tvv">${this._num(t.power_sensor)??"—"} <span class="u">kW</span></div></div>`:Y}
                </div>`:Y}
          </div>
        </div>

        ${t.solar_mode||this._modes.length?G`<div class="modes">
              ${t.solar_mode?G`<button class="mode ${g?"on":""}" @click=${e=>this._toggle(t.solar_mode,e)}>${xt("wb_sunny",18,g?"var(--g-amber-ink)":"var(--g-amber)")}Solar</button>`:Y}
              ${this._modes.map(t=>{const e=this._isOn(t.entity),s=t.name??this.hass.states[t.entity]?.attributes.friendly_name??t.entity;return G`<button class="mode ${e?"on":""}" @click=${e=>this._toggle(t.entity,e)}>${xt(t.icon??"bolt",18,e?"var(--g-amber-ink)":"var(--g-dim)")}${s}</button>`})}
            </div>`:Y}
      </div>
    `}};ie.styles=[$t,o`
    ...
    `],e([mt({attribute:!1})],ie.prototype,"hass",void 0),e([ft()],ie.prototype,"_config",void 0),ie=e([pt("glass-geyser-card")],ie);const ae={light:"lightbulb",switch:"toggle_on",fan:"mode_fan",input_boolean:"toggle_on",script:"play_arrow",automation:"bolt",scene:"palette"};let re=class extends ct{constructor(){super(...arguments),this._items=[]}setConfig(t){this._items=(t.entities??[]).map(t=>"string"==typeof t?{entity:t}:t),this._config={columns:4,...t}}getCardSize(){return Math.ceil(this._items.length/(this._config?.columns??4))+1}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{entities:[]}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!e||!this.hass||this._items.some(t=>e.states[t.entity]!==this.hass.states[t.entity])}_toggle(t){this.hass.callService("homeassistant","toggle",{entity_id:t})}render(){return this._config&&this.hass?this._items.length?G`
      <div class="card">
        ${this._config.title?G`<div class="hdr"><div class="hdr-l">${xt("grid_view",20,"var(--g-amber)")}<span class="title">${this._config.title}</span></div></div>`:Y}
        <div class="grid" style="grid-template-columns:repeat(${this._config.columns}, 1fr)">
          ${this._items.map(t=>{const e=this.hass.states[t.entity],s=e&&("on"===e.state||"open"===e.state||"home"===e.state||"playing"===e.state),i=t.entity.split(".")[0],a=t.icon??ae[i]??"circle",r=t.name??e?.attributes.friendly_name??t.entity,n=e?s?"On":"off"===e.state?"Off":e.state:"n/a";return G`
    ...
    `],e([mt({attribute:!1})],re.prototype,"hass",void 0),e([ft()],re.prototype,"_config",void 0),re=e([pt("glass-toggle-grid-card")],re);const ne=(t,e=!1)=>({entity:{...t?{domain:t}:{},...e?{multiple:!0}:{}}}),oe={text:{}},le=(t,e)=>({number:{min:t,max:e,mode:"box"}}),de=t=>({select:{mode:"dropdown",options:t}}),ce={"glass-person-card":[{name:"title",selector:oe},{name:"entities",required:!0,selector:ne("person",!0)}],"glass-weather-card":[{name:"entity",required:!0,selector:ne("weather")},{name:"sun",selector:ne("sun")},{name:"humidity",selector:ne("sensor")}],"glass-camera-card":[{name:"entity",required:!0,selector:ne("camera")},{name:"name",selector:oe},{name:"icon",selector:oe}],"glass-alarm-card":[{name:"entity",required:!0,selector:ne("alarm_control_panel")},{name:"variant",selector:de([{value:"shield",label:"Shield (default)"},{value:"radial",label:"Radial"},{value:"bar",label:"Compact bar"},{value:"keypad",label:"Keypad"},{value:"triggered",label:"Triggered / status"}])},{name:"name",selector:oe},{name:"subtitle",selector:oe},{name:"code",selector:oe},{name:"code_length",selector:le(1,8)},{name:"buttons",selector:{select:{multiple:!0,mode:"list",options:[{value:"disarm",label:"Disarm"},{value:"arm_home",label:"Arm Home"},{value:"arm_away",label:"Arm Away"},{value:"arm_night",label:"Arm Night"},{value:"arm_vacation",label:"Arm Vacation"}]}}}],"glass-media-card":[{name:"entity",required:!0,selector:ne("media_player")},{name:"name",selector:oe}],"glass-flight-card":[{name:"entity",required:!0,selector:ne("sensor")},{name:"title",selector:oe},{name:"max",selector:le(1,10)}],"glass-tile-card":[{name:"entity",required:!0,selector:ne()},{name:"name",selector:oe},{name:"icon",selector:oe}],"glass-energy-card":[{name:"title",selector:oe},{name:"variant",selector:de([{value:"flow",label:"Power flow (default)"},{value:"ring",label:"Battery ring"},{value:"stats",label:"Stat trio"},{value:"meters",label:"Meters"},{value:"production",label:"Production bars"}])},{name:"solar",selector:ne("sensor")},{name:"grid",selector:ne("sensor")},{name:"battery",selector:ne("sensor")},{name:"battery_soc",selector:ne("sensor")},{name:"house",selector:ne("sensor")},{name:"today",selector:ne("sensor")},{name:"imported",selector:ne("sensor")},{name:"exported",selector:ne("sensor")},{name:"saved",selector:ne("sensor")},{name:"meters",selector:ne("sensor",!0)},{name:"production",selector:ne("sensor")}],"glass-sensor-list-card":[{name:"title",selector:oe},{name:"entities",required:!0,selector:ne(["cover","binary_sensor"],!0)}],"glass-appliance-card":[{name:"name",selector:oe},{name:"subtitle",selector:oe},{name:"icon",selector:oe},{name:"icon_color",selector:{select:{mode:"dropdown",options:["cyan","amber","green","purple","red"]}}},{name:"status",selector:ne(["sensor","binary_sensor"])},{name:"remaining",selector:ne("sensor")},{name:"total",selector:ne("sensor")},{name:"toggle",selector:ne(["switch","input_boolean"])},{name:"stats",selector:ne(["sensor","binary_sensor","number"],!0)}],"glass-fridge-card":[{name:"name",selector:oe},{name:"subtitle",selector:oe},{name:"door",selector:ne("binary_sensor")},{name:"wifi",selector:ne("binary_sensor")},{name:"fridge_temp",selector:ne("number")},{name:"freezer_temp",selector:ne("number")},{name:"toggles",selector:ne(["switch","input_boolean"],!0)}],"glass-pool-card":[{name:"name",selector:oe},{name:"subtitle",selector:oe},{name:"switch",selector:ne(["switch","input_boolean"])},{name:"energy",selector:ne("sensor")},{name:"color",selector:de(["green","amber","cyan","purple","red"])}],"glass-stat-card":[{name:"entity",required:!0,selector:ne()},{name:"name",selector:oe},{name:"icon",selector:oe},{name:"color",selector:de(["amber","green","cyan","purple","red","dim"])}],"glass-lights-card":[{name:"title",selector:oe},{name:"entities",required:!0,selector:ne("light",!0)}],"glass-scenes-card":[{name:"title",selector:oe},{name:"columns",selector:le(2,6)},{name:"scenes",required:!0,selector:ne(["scene","script","automation"],!0)}],"glass-toggle-grid-card":[{name:"title",selector:oe},{name:"columns",selector:le(2,8)},{name:"entities",required:!0,selector:ne(["light","switch","fan","input_boolean","script","scene"],!0)}],"glass-light-card":[{name:"entity",required:!0,selector:ne("light")},{name:"name",selector:oe},{name:"subtitle",selector:oe}],"glass-aircon-card":[{name:"entity",required:!0,selector:ne("climate")},{name:"name",selector:oe},{name:"subtitle",selector:oe}],"glass-geyser-card":[{name:"name",selector:oe},{name:"subtitle",selector:oe},{name:"power",selector:ne(["switch","input_boolean","water_heater"])},{name:"current",selector:ne("sensor")},{name:"target",selector:ne(["number","water_heater","climate"])},{name:"min_temp",selector:le(0,100)},{name:"max_temp",selector:le(0,100)},{name:"solar",selector:ne("sensor")},{name:"power_sensor",selector:ne("sensor")},{name:"solar_mode",selector:ne(["switch","input_boolean"])},{name:"modes",selector:ne(["switch","input_boolean"],!0)}],"glass-dishwasher-card":[{name:"name",selector:oe},{name:"subtitle",selector:oe},{name:"status",selector:ne("sensor")},{name:"alert",selector:ne("binary_sensor")},{name:"alert_text",selector:oe},{name:"tiles",selector:ne(["binary_sensor","sensor"],!0)},{name:"levels",selector:ne("sensor",!0)},{name:"level_max",selector:le(1,10)}]},ge={entity:"Entity",entities:"Entities",title:"Title",name:"Name (optional)",icon:"Icon (optional)",sun:"Sun entity (for sunrise/sunset)",humidity:"Humidity sensor (optional)",code:"Arm/disarm code (optional)",max:"Max flights to show",buttons:"Action buttons to show",variant:"Layout / style",code_length:"Keypad code length",today:"Solar-today energy sensor (big number)",imported:"Imported-today sensor",exported:"Exported-today sensor",saved:"Money-saved sensor",meters:"Percentage sensors to show as meters",production:"Production sensor for hourly bars (needs statistics)",subtitle:"Subtitle (e.g. room/area)",icon_color:"Icon accent colour",status:"Status sensor (drives the pill)",remaining:"Remaining-time sensor (timestamp → progress ring)",total:"Total-time sensor (minutes, for the ring %)",toggle:"Power switch (optional on/off button)",stats:"Extra sensors shown as stat tiles",door:"Door sensor",wifi:"Wi-Fi / connectivity sensor (optional)",fridge_temp:"Fridge temperature (number entity)",freezer_temp:"Freezer temperature (number entity)",toggles:"Feature switches (Express cool/mode, etc.)",switch:"Pump switch",energy:"Energy sensor (kWh)",color:"Accent colour",columns:"Columns",scenes:"Scenes / scripts to show",alert:"Alert sensor (e.g. refill needed)",alert_text:"Alert message",tiles:"Status tiles",levels:"Level sensors (shown as bars)",level_max:"Level bar maximum",power:"Power switch (on/off)",current:"Current temperature sensor",target:"Target temperature (number / thermostat)",min_temp:"Minimum temperature",max_temp:"Maximum temperature",solar:"Solar collector temperature sensor",power_sensor:"Power draw sensor (kW)",solar_mode:"Solar mode switch (toggleable)",modes:"Extra mode switches (boost, element, timer)",solar:"Solar power sensor",grid:"Grid power sensor (+import / −export)",battery:"Battery power sensor (+discharge / −charge)",battery_soc:"Battery charge % sensor",house:"House load sensor (optional; derived if empty)"},pe={icon:"Material Symbols name, e.g. lightbulb, garage, kitchen",code:"Only if your panel requires a code to arm/disarm"};let he=class extends ct{constructor(){super(...arguments),this._label=t=>ge[t.name]??t.name,this._helper=t=>pe[t.name]??""}setConfig(t){this._config=t}_valueChanged(t){if(t.stopPropagation(),!this._config)return;const e={...this._config,...t.detail.value,type:this._config.type};_t(this,"config-changed",{config:e})}render(){if(!this.hass||!this._config)return Y;const t=this._config.type.replace(/^custom:/,""),e=ce[t];return e?G`
      <ha-form
        .hass=${this.hass}
        .data=${this._config}
        .schema=${e}
