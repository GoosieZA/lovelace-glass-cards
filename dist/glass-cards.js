...
`;let $t=class extends dt{setConfig(t){this._config={title:"Home",entities:[],...t}}getCardSize(){return 2}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{title:"Home",entities:[]}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||this._config.entities.some(t=>e.states[t]!==this.hass.states[t])}_statusOf(t){const e=this.hass?.states[t];if(!e)return{label:"Unknown",home:!1,accent:"var(--g-dim)"};if("home"===e.state)return{label:"Home",home:!0,accent:"var(--g-green)"};if("not_home"===e.state)return{label:"Away",home:!1,accent:"var(--g-dim)"};return{label:e.state.charAt(0).toUpperCase()+e.state.slice(1),home:!1,accent:"var(--g-amber)"}}_relTime(t){if(!t)return"";const e=Math.round((Date.now()-new Date(t).getTime())/6e4);if(e<1)return"just now";if(e<60)return`${e}m ago`;const i=Math.round(e/60);return i<24?`${i}h ago`:`${Math.round(i/24)}d ago`}_open(t){xt(this,"hass-more-info",{entityId:t})}render(){if(!this._config||!this.hass)return Y;if(!this._config.entities.length)return yt("Add person entities","group");const t=this._config.entities.map(t=>({id:t,st:this.hass.states[t]})),e=t.filter(t=>"home"===t.st?.state).length;return F`
      <div class="card">
        ...
        <div class="people">
          ${t.map(t=>{if(!t.st)return F`<div class="missing">${t.id} not found</div>`;const e=this._statusOf(t.id),i=t.st.attributes.entity_picture,s=t.st.attributes.friendly_name??t.id,a=s.split(" ").map(t=>t[0]).slice(0,2).join("").toUpperCase();return F`
              <button class="person" @click=${()=>this._open(t.id)} title=${s}>
                <div class="avatar" style="--accent:${e.accent}">
                  ${i?F`<img src=${i} alt=${s} />`:F`<span class="mono-av">${a}</span>`}
                  ${e.home?F`<span class="ring"></span>`:Y}
                </div>
                <div class="who">
        ...
        <div class="top">
          <div class="cond">
            ...
            <div>
              ...
              ${n?F`<div class="wind">Wind ${n}</div>`:Y}
            </div>
          </div>
    ...
    `],e([mt({attribute:!1})],Ct.prototype,"hass",void 0),e([ft()],Ct.prototype,"_config",void 0),Ct=e([pt("glass-weather-card")],Ct);let zt=class extends dt{setConfig(t){this._config={icon:"videocam",...t}}getCardSize(){return 3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{entity:"camera.garage",icon:"garage"}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||e.states[this._config.entity]!==this.hass.states[this._config.entity]}_stamp(t){const e=t?new Date(t):new Date,i=t=>String(t).padStart(2,"0");return`${e.getFullYear()}-${i(e.getMonth()+1)}-${i(e.getDate())} · ${i(e.getHours())}:${i(e.getMinutes())}:${i(e.getSeconds())}`}render(){if(!this._config||!this.hass)return Y;const t=this._config.entity?this.hass.states[this._config.entity]:void 0;if(!t)return yt("Select a camera entity","videocam");const e=t.attributes.entity_picture,i=this._config.name??t.attributes.friendly_name??this._config.entity,s="streaming"===t.state||"recording"===t.state||"idle"===t.state;return F`
      <div class="cam" @click=${()=>xt(this,"hass-more-info",{entityId:this._config.entity})}>
        ${e?F`<img src=${e} alt=${i} />`:Y}
        <div class="scan"></div>
        ${!1===this._config.show_timestamp?Y:F`<div class="ts">${this._stamp(t.last_changed)}</div>`}
        ${s?F`<div class="live-badge"><span class="rec"></span>LIVE</div>`:F`<div class="live-badge off">OFFLINE</div>`}
        ${e?Y:F`<div class="ph">${_t("videocam",52)}</div>`}
    ...
    `}_renderShield(){const t=this._look(this._st.state),e=this._config.name??this._st.attributes.friendly_name??"Security";return F`<div class="card" style="border-color:${t.border}">
      ${this._header(t,e)}
      <div class="actions">${this._actionButtons()}</div>
    </div>`}_renderRadial(){const t=this._look(this._st.state),e=this._config.name??this._st.attributes.friendly_name??"Security";return F`<div class="card center" style="border-color:${t.border}">
      ...
      <div class="actions">${this._actionButtons()}</div>
    </div>`}_renderBar(){const t=this._look(this._st.state),e=this._config.name??this._st.attributes.friendly_name??"Security",i=this._st.state.startsWith("armed")||"triggered"===this._st.state;return F`<div class="bar" style="border-color:${t.border}">
    ...
    </div>`}_renderKeypad(){const t=this._config.code_length??4,e=Array.from({length:t},(t,e)=>e<this._code.length?"●":"○").join(" "),i=this._look(this._st.state);return F`<div class="card">
      <div class="disp"><span class="ms" style="font-size:22px;color:${i.accent}">${i.glyph}</span><div class="dots">${e}</div></div>
      <div class="pad">
        ${["1","2","3","4","5","6","7","8","9","back","0","ok"].map(t=>"back"===t?F`<button class="k" @click=${()=>this._key("back")}>${_t("backspace",20,"var(--g-dim)")}</button>`:"ok"===t?F`<button class="k ok" @click=${()=>this._key("ok")}>${_t("check",20,"var(--g-amber-ink)")}</button>`:F`<button class="k" @click=${()=>this._key(t)}>${t}</button>`)}
      </div>
    </div>`}_renderTriggered(){if("triggered"===this._st.state)return F`<div class="alert red">
      ...
      </div>`;const t=this._look(this._st.state);return F`<div class="alert" style="border-color:${t.border}">
    ...
    `],e([mt({attribute:!1})],Nt.prototype,"hass",void 0),e([ft()],Nt.prototype,"_config",void 0),e([ft()],Nt.prototype,"_code",void 0),Nt=e([pt("glass-alarm-card")],Nt);const Mt=[["#7dd68a","var(--g-green)"],["#e0b24a","var(--g-amber)"],["#a97ff0","var(--g-purple)"],["#5ab9bd","var(--g-cyan)"]];let Pt=class extends dt{constructor(){super(...arguments),this._bars=[]}setConfig(t){this._config={title:"Energy",variant:"flow",...t},this._fetchedFor=void 0}getCardSize(){return"flow"===this._config?.variant||"production"===this._config?.variant?4:3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{title:"Energy",variant:"flow",solar:"sensor.solar_power",grid:"sensor.grid_power",battery:"sensor.battery_power"}}shouldUpdate(t){if(t.has("_config")||t.has("_bars"))return!0;const e=t.get("hass");if(!e||!this.hass||!this._config)return!0;const i=this._config;return[i.solar,i.grid,i.battery,i.battery_soc,i.house,i.today,i.imported,i.exported,i.saved,i.production,...i.meters??[]].filter(Boolean).some(t=>e.states[t]!==this.hass.states[t])}updated(){if(!this.hass||!this._config||"production"!==this._config.variant||!this._config.production)return;const t=this.hass.states[this._config.production];t&&t.last_updated!==this._fetchedFor&&(this._fetchedFor=t.last_updated,this._loadProduction(this._config.production))}async _loadProduction(t){const e=new Date;e.setHours(0,0,0,0);try{const i=(await this.hass.callWS({type:"recorder/statistics_during_period",start_time:e.toISOString(),statistic_ids:[t],period:"hour"}))[t]??[];this._bars=i.map(t=>({v:Math.max(0,Number(t.change??t.state??0)),label:String(new Date(Number(t.start)).getHours())}))}catch{this._bars=[]}}_watts(t){if(!t||!this.hass)return null;const e=this.hass.states[t];if(!e||"unavailable"===e.state||"unknown"===e.state)return null;const i=Number(e.state);if(Number.isNaN(i))return null;return"kw"===String(e.attributes.unit_of_measurement??"").toLowerCase()?1e3*i:i}_kw(t){return(t/1e3).toFixed(1)}_stateNum(t){const e=t?Number(this.hass.states[t]?.state):NaN;return Number.isNaN(e)?null:e}_label(t){return this.hass.states[t]?.attributes.friendly_name??t}render(){if(!this._config||!this.hass)return Y;switch(this._config.variant){case"ring":return this._renderRing();case"stats":return this._renderStats();case"meters":return this._renderMeters();case"production":return this._renderProduction();default:return this._renderFlow()}}_head(t){return F`<div class="hdr"><div class="hdr-l">${_t("bolt",22,"var(--g-amber)")}<span class="title">${this._config.title}</span></div>${t??Y}</div>`}_renderFlow(){const t=this._watts(this._config.solar),e=this._watts(this._config.grid),i=this._watts(this._config.battery);let s=this._watts(this._config.house);null==s&&(s=(t??0)+(e??0)+(i??0));const a=this._stateNum(this._config.battery_soc),r=(t??0)>20,n=(e??0)>20,o=(e??0)<-20,l=(i??0)>20,c=(i??0)<-20,d=o?{label:"Exporting",purple:!1}:n?{label:"Grid import",purple:!0}:{label:"Self-powered",purple:!1},g=(t,e,i=!1)=>t?`stroke:${e};animation:g-flow 0.7s linear infinite${i?" reverse":""};`:"stroke:rgba(255,255,255,0.06);";return F`<div class="card">
      ...
      <div class="flow">
        ...
        ${null!=t?F`<div class="node solar"><div class="chip amber">${_t("solar_power",20,"var(--g-amber)")}<span>${this._kw(t)}</span></div><span class="cap">Solar · kW</span></div>`:Y}
        <div class="node home">${_t("home",26,"var(--g-amber)")}<span class="watts t-num">${Math.round(s)}</span><span class="cap">Watts now</span></div>
        ${null!=e?F`<div class="node grid"><div class="chip">${_t("bolt",20,"var(--g-purple)")}<span style="color:var(--g-purple)">${this._kw(Math.abs(e))}</span></div><span class="cap">Grid · kW${n?" (import)":o?" (export)":" (idle)"}</span></div>`:Y}
        ${null!=i?F`<div class="node batt"><div class="chip green">${_t(c?"battery_charging_full":"battery_full",20,"var(--g-green)")}<span style="color:var(--g-green)">${this._kw(Math.abs(i))}</span></div><span class="cap">Battery${null!=a?` · ${Math.round(a)}%`:" · kW"}</span></div>`:Y}
      </div>
    </div>`}_renderRing(){const t=this._config,e=this._stateNum(t.battery_soc)??0,i=this._watts(t.solar);let s=this._watts(t.house);null==s&&(s=(i??0)+(this._watts(t.grid)??0)+(this._watts(t.battery)??0));const a=this._watts(t.battery),r=null!=a&&Math.abs(a)>5?F`<div class="delta" style="color:${a<0?"var(--g-green)":"var(--g-amber)"}">${a<0?"↓":"↑"} ${Math.abs(Math.round(a))} W</div>`:Y;return F`<div class="card">
      ${this._head()}
      <div class="ring-row">
      ...
      ${t.today||t.grid?F`<div class="ring-tiles">
            ${t.today?F`<div class="stat"><div class="t-num sv">${this._fmt(t.today).val} <span class="su">${this._fmt(t.today).unit||"kWh"}</span></div><div class="sc">Solar today</div></div>`:Y}
            ${t.grid?F`<div class="stat"><div class="t-num sv">${(this._watts(t.grid)??0)>5?"Import":(this._watts(t.grid)??0)<-5?"Export":"On"} <span style="color:var(--g-green)">●</span></div><div class="sc">Grid status</div></div>`:Y}
          </div>`:Y}
      ...
      ${i.length?F`<div class="tri">
            ${i.map(t=>{const e=this._fmt(t.id);return F`<div class="stat"><div class="t-num sv" style="color:${t.color}">${e.val}${e.unit?F` <span class="su">${e.unit}</span>`:Y}</div><div class="sc">${this._label(t.id)}</div></div>`})}
          </div>`:Y}
    </div>`}_renderMeters(){const t=this._config,e=[...t.battery_soc?[t.battery_soc]:[],...t.meters??[]];return F`<div class="card">
      ${this._head()}
      <div class="meters">
        ${e.map((e,i)=>{const s=Math.max(0,Math.min(100,this._stateNum(e)??0)),[a,r]=Mt[i%Mt.length];return F`<div class="m">
    ...
    </div>`}_renderProduction(){const t=this._bars.reduce((t,e)=>t+e.v,0),e=Math.max(.001,...this._bars.map(t=>t.v));return F`<div class="card">
      ${this._head(F`<div class="t-num total">${t.toFixed(1)} <span class="su">kWh</span></div>`)}
      ${this._bars.length?F`<div class="bars">
            ${this._bars.map(t=>F`<div class="bar-col"><div class="bar" style="height:${t.v/e*100}%;background:${t.v>=.66*e?"var(--g-amber)":"var(--g-amber-deep)"}"></div><span class="bl">${t.label}</span></div>`)}
          </div>`:F`<div class="empty">No production data for today yet.</div>`}
    </div>`}};Pt.styles=[wt,o`
    ...
    `],e([mt({attribute:!1})],Ut.prototype,"hass",void 0),e([ft()],Ut.prototype,"_config",void 0),Ut=e([pt("glass-tile-card")],Ut);let Tt=class extends dt{constructor(){super(...arguments),this._rows=[]}setConfig(t){this._rows=(t.entities??[]).map(t=>"string"==typeof t?{entity:t}:t),this._config={title:"Sensors",...t}}getCardSize(){return this._rows.length+1}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{title:"Sensors",entities:[]}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!e||!this.hass||this._rows.some(t=>e.states[t.entity]!==this.hass.states[t.entity])}_resolveType(t){if(t.type&&"auto"!==t.type)return t.type;const e=this.hass.states[t.entity],i=e?.attributes.device_class;return t.entity.startsWith("cover.")||"garage"===i?"garage":"window"===i?"window":"door"}_isOpen(t){const e=this.hass.states[t];return!!e&&("open"===e.state||"on"===e.state||"opening"===e.state)}_visual(t,e){return"garage"===t?F`<div class="v garage">
        <div class="slats" style="transform:translateY(${e?"-100%":"0"})">
          <span></span><span></span><span></span><span></span>
        </div>
        <div class="floor"></div>
      </div>`:"window"===t?F`<div class="v window">
        <div class="pane top" style="transform:translateY(${e?"-60%":"0"})"></div>
        <div class="pane bot"></div>
      </div>`:F`<div class="v door">
      <div class="frame"><div class="leaf" style="transform:rotateY(${e?"-62deg":"0deg"})"><span class="knob"></span></div></div>
    </div>`}render(){if(!this._config||!this.hass)return Y;if(!this._rows.length)return yt("Add door / window / cover sensors","sensors");const t=this._rows.filter(t=>this._isOpen(t.entity)).length;return F`
      <div class="card">
        <div class="hdr">
        ...
        ${this._rows.map((t,e)=>{const i=this.hass.states[t.entity];if(!i)return F`<div class="line"><div class="missing">${t.entity} not found</div></div>`;const s=this._resolveType(t),a=this._isOpen(t.entity),r=t.name??i.attributes.friendly_name??t.entity;return F`
    ...
    `],e([mt({attribute:!1})],Ht.prototype,"hass",void 0),e([ft()],Ht.prototype,"_config",void 0),Ht=e([pt("glass-media-card")],Ht);let Wt=class extends dt{setConfig(t){this._config={title:"Overhead",max:4,...t}}getCardSize(){return 2}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{entity:"sensor.flightradar24_current_in_area",title:"Overhead"}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||e.states[this._config.entity]!==this.hass.states[this._config.entity]}_route(t){const e=t.airport_origin_code_iata||t.airport_origin_city,i=t.airport_destination_code_iata||t.airport_destination_city;return e&&i?`${e} → ${i}`:e||i||""}render(){if(!this._config||!this.hass)return Y;const t=this._config.entity?this.hass.states[this._config.entity]:void 0;if(!t)return yt("Select a Flightradar24 sensor","flight");const e=t.attributes.flights??[],i=e.length;return F`
      <div class="card" @click=${()=>xt(this,"hass-more-info",{entityId:this._config.entity})}>
        ...
        ${0===i?F`<div class="empty">${_t("travel_explore",30,"var(--g-faint)")}<span>No aircraft in your area</span></div>`:e.slice(0,this._config.max).map(t=>{const e=t.callsign||t.flight_number||"Unknown",i=[null!=t.altitude?`${Math.round(Number(t.altitude)).toLocaleString()} ft`:null,null!=t.ground_speed?`${Math.round(Number(t.ground_speed))} kt`:null,t.aircraft_code||t.aircraft_model].filter(Boolean);return F`
                <div class="row">
                  <div class="plane" style=${null!=t.heading?`transform:rotate(${Number(t.heading)}deg)`:""}>
                    ${_t("flight",20,"var(--g-cyan)")}
                  </div>
                  <div class="info">
                    <div class="cs">${e}${this._route(t)?F`<span class="rt">${this._route(t)}</span>`:Y}</div>
                    ${i.length?F`<div class="det">${i.join(" · ")}</div>`:Y}
                  </div>
                  ${null!=t.distance?F`<span class="dist">${Number(t.distance).toFixed(1)} km</span>`:Y}
                </div>
              `})}
      </div>
    `}};Wt.styles=[wt,o`
      ...
      <div class="card">
        ...
        ${a?F`
              <div class="active">
                ...
                <div class="stat-rows">
                  ${r.map(t=>F`<div class="srow"><div class="t-num sval">${this._statValue(t)}</div><div class="scap">${this._statLabel(t)}</div></div>`)}
                </div>
              </div>
            `:r.length?F`<div class="stat-grid">
                ${r.map(t=>F`<div class="stile"><div class="t-num sval">${this._statValue(t)}</div><div class="scap">${this._statLabel(t)}</div></div>`)}
              </div>`:Y}

        ${t.toggle?F`<button class="power ${n?"on":""}" @click=${this._toggle}>
    ...
    `],e([mt({attribute:!1})],Bt.prototype,"hass",void 0),e([ft()],Bt.prototype,"_config",void 0),Bt=e([pt("glass-appliance-card")],Bt);let Dt=class extends dt{constructor(){super(...arguments),this._toggles=[]}setConfig(t){this._toggles=(t.toggles??[]).map(t=>"string"==typeof t?{entity:t}:t),this._config={name:"Refrigerator",subtitle:"Kitchen",...t}}getCardSize(){return 3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{name:"Refrigerator",subtitle:"Kitchen",door:"",fridge_temp:"",freezer_temp:""}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");if(!e||!this.hass||!this._config)return!0;const i=[this._config.door,this._config.wifi,this._config.fridge_temp,this._config.freezer_temp,...this._toggles.map(t=>t.entity)].filter(Boolean);return i.some(t=>e.states[t]!==this.hass.states[t])}_num(t){const e=t?this.hass.states[t]:void 0;if(!e)return null;const i=Number(e.state);return{value:Number.isNaN(i)?null:i,min:Number(e.attributes.min??-30),max:Number(e.attributes.max??30),step:Number(e.attributes.step??1)}}_step(t,e,i){i.stopPropagation();const s=this._num(t);if(!t||!s||null==s.value)return;const a=Math.min(s.max,Math.max(s.min,s.value+e*s.step));this.hass.callService("number","set_value",{entity_id:t,value:a})}_toggle(t,e){e.stopPropagation(),this.hass.callService("homeassistant","toggle",{entity_id:t})}_tempTile(t,e,i,s){const a=this._num(s);return F`
      <div class="tt">
        ...
        ${a?F`<div class="range">range ${a.min}…${a.max} °C</div>`:Y}
      </div>
    `}render(){if(!this._config||!this.hass)return Y;const t=this._config,e=!!t.door&&"on"===this.hass.states[t.door]?.state,i=t.wifi?"on"===this.hass.states[t.wifi]?.state:null;return F`
      <div class="card">
        <div class="head">
          ...
          <div class="who">
            ...
            <div class="sub">
              ${t.subtitle}${null!=i?F` · ${_t("wifi",14,i?"var(--g-green)":"var(--g-dim)")}${i?"Connected":"Offline"}`:Y}
            </div>
          </div>
          ${t.door?F`<button class="pill ${e?"open":"closed"}" @click=${()=>xt(this,"hass-more-info",{entityId:t.door})}>
        ...
        ${this._toggles.length?F`<div class="rows">
              ${this._toggles.map(e=>{const i=this.hass.states[e.entity],s="on"===i?.state;let a=e.name??i?.attributes.friendly_name??e.entity;t.name&&a.toLowerCase().startsWith(t.name.toLowerCase()+" ")&&(a=a.slice(t.name.length+1));const r=e.icon??(/cool/i.test(a)?"mode_cool":"bolt");return F`
                  <div class="row">
    ...
    `],e([mt({attribute:!1})],Dt.prototype,"hass",void 0),e([ft()],Dt.prototype,"_config",void 0),Dt=e([pt("glass-fridge-card")],Dt);const Gt={green:"var(--g-green)",amber:"var(--g-amber)",cyan:"var(--g-cyan)",purple:"var(--g-purple)",red:"var(--g-red-text)"};let Ft=class extends dt{setConfig(t){this._config={name:"Pool Pump",subtitle:"Backyard",...t}}getCardSize(){return 3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{name:"Pool Pump",subtitle:"Backyard",switch:"",energy:""}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||[this._config.switch,this._config.energy].filter(Boolean).some(t=>e.states[t]!==this.hass.states[t])}_toggle(t){t.stopPropagation(),this._config.switch&&this.hass.callService("homeassistant","toggle",{entity_id:this._config.switch})}render(){if(!this._config||!this.hass)return Y;const t=this._config,e=t.switch?this.hass.states[t.switch]:void 0,i=!e||"unavailable"===e.state,s="on"===e?.state,a=Gt[t.color??"green"]??"var(--g-green)",r=i?"var(--g-dim)":s?a:"var(--g-amber)",n=i?"Unavailable":s?"Running":"Idle",o=t.energy?this.hass.states[t.energy]:void 0,l=o&&!Number.isNaN(Number(o.state))?Number(o.state).toFixed(1):"—",c=o?.attributes.unit_of_measurement??"kWh",d=s?"animation:g-spin 1.1s linear infinite;transform-origin:48px 48px;":"",g=s?"animation:g-spin 6s linear infinite;transform-origin:48px 48px;":"";return F`
      <div class="card" style="border-color:${s?`color-mix(in srgb, ${a} 30%, transparent)`:"var(--g-hair)"}">
        <div class="head">
          ...
          <div class="who">
            <div class="title">${t.name}${s?F` <span class="live">LIVE</span>`:Y}</div>
    ...
    `],e([mt({attribute:!1})],Ft.prototype,"hass",void 0),e([ft()],Ft.prototype,"_config",void 0),Ft=e([pt("glass-pool-card")],Ft);const qt={door:"door_front",window:"window",connectivity:"wifi",running:"check_circle"};let Vt=class extends dt{setConfig(t){this._config={name:"Dishwasher",subtitle:"Kitchen",level_max:4,...t}}getCardSize(){return 3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{name:"Dishwasher",subtitle:"Kitchen",status:""}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");if(!e||!this.hass||!this._config)return!0;const i=this._config;return[i.status,i.alert,...i.tiles??[],...i.levels??[]].filter(Boolean).some(t=>e.states[t]!==this.hass.states[t])}_label(t){let e=this.hass.states[t]?.attributes.friendly_name??t;const i=this._config.name;return i&&e.toLowerCase().startsWith(i.toLowerCase()+" ")&&(e=e.slice(i.length+1)),e}_pretty(t){return t.replace(/_/g," ").replace(/^\w/,t=>t.toUpperCase())}_statusLook(t){const e=t.toLowerCase(),i=this._pretty(t);return/(run|washing|active|start)/.test(e)?{color:"var(--g-cyan)",label:i}:/(complete|finish|clean|done)/.test(e)?{color:"var(--g-green)",label:i}:/(pause|delay)/.test(e)?{color:"var(--g-amber)",label:i}:/(off|idle|power_off|standby)/.test(e)?{color:"var(--g-dim)",label:"Power off"}:{color:"var(--g-dim)",label:i}}_tile(t){const e=this.hass.states[t];if(!e)return Y;const i=e.attributes.device_class,s=t.split(".")[0];let a,r=!1;"binary_sensor"===s||"switch"===s?(r="on"===e.state,a="door"===i||"window"===i||"opening"===i?r?"Open":"Closed":"connectivity"===i?r?"Online":"Offline":r?"On":"Off"):a=this._pretty(e.state);return F`<div class="tile" @click=${()=>xt(this,"hass-more-info",{entityId:t})}>
      ...
      <div class="card">
        <div class="head">
          ...
          ${i?F`<span class="pill" style="color:${i.color}"><span class="dot" style="background:${i.color}"></span>${i.label}</span>`:Y}
        </div>

        ${s?F`<div class="alert">${_t("opacity",22,"var(--g-amber)")}<div><div class="a-t">${t.alert_text??"Attention needed"}</div></div></div>`:Y}

        ${a.length?F`<div class="tiles">${a.slice(0,3).map(t=>this._tile(t))}</div>`:Y}
        ${r.length?F`<div class="levels">${r.map(t=>this._level(t))}</div>`:Y}
      </div>
    `}};Vt.styles=[wt,o`
    ...
    `],e([mt({attribute:!1})],Kt.prototype,"hass",void 0),e([ft()],Kt.prototype,"_config",void 0),Kt=e([pt("glass-stat-card")],Kt);let Xt=class extends dt{constructor(){super(...arguments),this._groups=[]}setConfig(t){t.groups?.length?this._groups=t.groups:t.entities?.length?this._groups=[{entities:t.entities}]:this._groups=[],this._config={title:"Lights",...t}}getCardSize(){return 1+this._groups.reduce((t,e)=>t+Math.ceil(e.entities.length/3),0)}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{title:"Lights",entities:[]}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!e||!this.hass||this._all().some(t=>e.states[t]!==this.hass.states[t])}_all(){return this._groups.flatMap(t=>t.entities)}_isOn(t){return"on"===this.hass.states[t]?.state}_toggle(t,e){e.stopPropagation(),this.hass.callService("homeassistant","toggle",{entity_id:t})}_master(t,e){e.stopPropagation();const i=t.entities.some(t=>this._isOn(t));this.hass.callService("light",i?"turn_off":"turn_on",{entity_id:t.entities})}_name(t){return this.hass.states[t]?.attributes.friendly_name??t}render(){if(!this._config||!this.hass)return Y;if(!this._all().length)return yt("Add lights","lightbulb");const t=this._all().filter(t=>this._isOn(t)).length;return F`
      <div class="card">
        ...
        ${this._groups.map(t=>{const e=t.entities.filter(t=>this._isOn(t)).length,i=e>0;return F`
            <div class="group">
              ${t.name?F`<div class="g-hdr">
              ...
              <div class="tiles">
                ${t.entities.map(t=>{const e=this._isOn(t),i=this.hass.states[t],s=e&&null!=i?.attributes.brightness?`${Math.round(Number(i.attributes.brightness)/255*100)}%`:e?"On":"Off";return F`
    ...
    `],e([mt({attribute:!1})],Xt.prototype,"hass",void 0),e([ft()],Xt.prototype,"_config",void 0),Xt=e([pt("glass-lights-card")],Xt);const Qt={scene:"palette",script:"play_arrow",automation:"bolt"};let Zt=class extends dt{constructor(){super(...arguments),this._scenes=[]}setConfig(t){this._scenes=(t.scenes??[]).map(t=>"string"==typeof t?{entity:t}:t),this._config={columns:4,...t}}getCardSize(){return Math.ceil(this._scenes.length/(this._config?.columns??4))+1}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{scenes:[]}}shouldUpdate(){return!0}_activate(t){const e=t.split(".")[0],i="scene"===e?"scene.turn_on":"script"===e?"script.turn_on":"automation"===e?"automation.trigger":"homeassistant.turn_on",[s,a]=i.split(".");this.hass.callService(s,a,{entity_id:t})}render(){return this._config&&this.hass?this._scenes.length?F`
      <div class="card">
        ${this._config.title?F`<div class="hdr"><div class="hdr-l">${_t("auto_awesome",20,"var(--g-amber)")}<span class="title">${this._config.title}</span></div></div>`:Y}
        <div class="grid" style="grid-template-columns:repeat(${this._config.columns}, minmax(0, 1fr))">
          ${this._scenes.map(t=>{const e=this.hass.states[t.entity],i=t.name??e?.attributes.friendly_name??t.entity,s=t.icon??Qt[t.entity.split(".")[0]]??"palette";return F`<button class="scene" @click=${()=>this._activate(t.entity)}>${_t(s,24,"var(--g-text-hi)")}<span class="sn">${i}</span></button>`})}
        </div>
      </div>
    `:yt("Add scenes or scripts","auto_awesome"):Y}};Zt.styles=[wt,o`
      .card { display: flex; flex-direction: column; gap: 14px; }
    ...
    `],e([mt({attribute:!1})],Zt.prototype,"hass",void 0),e([ft()],Zt.prototype,"_config",void 0),Zt=e([pt("glass-scenes-card")],Zt);const Jt=[[255,92,92],[243,208,106],[185,246,166],[135,221,225],[130,170,255],[201,166,255],[255,150,200],[255,244,224]];let te=class extends dt{setConfig(t){this._config=t}getCardSize(){return 4}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{entity:""}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||e.states[this._config.entity]!==this.hass.states[this._config.entity]}get _st(){return this.hass.states[this._config.entity]}_modes(){return this._st?.attributes.supported_color_modes??[]}_brightPct(){const t=this._st?.attributes.brightness;return null!=t?Math.round(Number(t)/255*100):0}_rgb(){const t=this._st?.attributes.rgb_color;return t&&3===t.length?[t[0],t[1],t[2]]:null}_clickPct(t){const e=t.currentTarget.getBoundingClientRect();return Math.max(0,Math.min(1,(t.clientX-e.left)/e.width))}_toggle(t){t.stopPropagation(),this.hass.callService("light","toggle",{entity_id:this._config.entity})}_setBright(t){const e=Math.max(1,Math.round(100*this._clickPct(t)));this.hass.callService("light","turn_on",{entity_id:this._config.entity,brightness_pct:e})}_setTemp(t){const e=Number(this._st.attributes.min_color_temp_kelvin??2e3),i=Number(this._st.attributes.max_color_temp_kelvin??6500),s=Math.round(e+this._clickPct(t)*(i-e));this.hass.callService("light","turn_on",{entity_id:this._config.entity,color_temp_kelvin:s})}_setRgb(t){this.hass.callService("light","turn_on",{entity_id:this._config.entity,rgb_color:t})}render(){if(!this._config||!this.hass)return Y;if(!this._st)return yt("Select a light","lightbulb");const t="on"===this._st.state,e=this._modes(),i=t&&e.some(t=>["brightness","color_temp","hs","rgb","rgbw","rgbww","xy","white"].includes(t)),s=t&&e.includes("color_temp"),a=t&&e.some(t=>["hs","rgb","rgbw","rgbww","xy"].includes(t)),r=this._rgb(),n=this._brightPct(),o=this._config.name??this._st.attributes.friendly_name??this._config.entity,l=t?r?`rgb(${r.join(",")})`:"var(--g-amber)":"var(--g-faint)",c=t?`radial-gradient(circle at 50% 45%, ${r?`rgba(${r.join(",")},0.35)`:"rgba(243,208,106,0.3)"} 0%, transparent 70%)`:"transparent";return F`
      <div class="card">
        ...
        ${a?F`<div class="swatches">
              ${Jt.map(t=>F`<button class="sw" style="background:rgb(${t.join(",")})" @click=${()=>this._setRgb(t)}></button>`)}
            </div>`:Y}
      </div>
    `}};te.styles=[wt,o`
      .card { display: flex; flex-direction: column; gap: 18px; }
    ...
    `],e([mt({attribute:!1})],te.prototype,"hass",void 0),e([ft()],te.prototype,"_config",void 0),te=e([pt("glass-light-card")],te);const ee={off:{icon:"power_settings_new",color:"var(--g-dim)"},cool:{icon:"ac_unit",color:"var(--g-cyan)"},heat:{icon:"local_fire_department",color:"var(--g-amber)"},dry:{icon:"water_drop",color:"var(--g-purple)"},fan_only:{icon:"mode_fan",color:"var(--g-cyan)"},auto:{icon:"autorenew",color:"var(--g-green)"},heat_cool:{icon:"device_thermostat",color:"var(--g-green)"}};let ie=class extends dt{setConfig(t){this._config=t}getCardSize(){return 3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{entity:""}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||e.states[this._config.entity]!==this.hass.states[this._config.entity]}get _st(){return this.hass.states[this._config.entity]}_setTemp(t,e){e.stopPropagation();const i=this._st.attributes,s=Number(i.target_temp_step??.5),a=Number(i.temperature);if(Number.isNaN(a))return;const r=Number(i.min_temp??7),n=Number(i.max_temp??35),o=Math.min(n,Math.max(r,a+t*s));this.hass.callService("climate","set_temperature",{entity_id:this._config.entity,temperature:o})}_setMode(t){this.hass.callService("climate","set_hvac_mode",{entity_id:this._config.entity,hvac_mode:t})}_setFan(t){this.hass.callService("climate","set_fan_mode",{entity_id:this._config.entity,fan_mode:t})}_toggle(t){t.stopPropagation();if("off"!==this._st.state)this._setMode("off");else{const t=this._st.attributes.hvac_modes??[];this._setMode(t.find(t=>"off"!==t)??"cool")}}render(){if(!this._config||!this.hass)return Y;if(!this._st)return yt("Select a climate entity","mode_fan");const t=this._st.attributes,e=this._st.state,i="off"!==e,s=ee[e]??ee.off,a=this._config.name??t.friendly_name??"Aircon",r=t.current_temperature,n=null!=t.temperature?Number(t.temperature):null,o=t.hvac_action??e,l=(t.hvac_modes??[]).filter(t=>"off"!==t),c=t.fan_modes??[];return F`
      <div class="card">
        ...
        ${l.length?F`<div class="seg">
              ${l.map(t=>F`<button class="sg ${e===t?"on":""}" title=${t} @click=${()=>this._setMode(t)}>${_t((ee[t]??{icon:"circle"}).icon,19)}</button>`)}
            </div>`:Y}

        ${c.length?F`<div class="fan-row">
              ${_t("air",18,"var(--g-dim)")}
              <div class="seg fan">
                ${c.map(e=>F`<button class="sg txt ${t.fan_mode===e?"on":""}" @click=${()=>this._setFan(e)}>${e}</button>`)}
              </div>
            </div>`:Y}
      </div>
    `}};ie.styles=[wt,o`
      ...
      <div class="card">
        ...
        <div class="body">
          <div class="tank">
            ...
            ${o?F`<span class="rise" style="left:24px"></span><span class="rise" style="left:44px;animation-delay:.5s"></span><span class="rise" style="left:62px;animation-delay:1s"></span>`:Y}
          ...
          <div class="right">
            ...
            ${t.solar||t.power_sensor?F`<div class="tiles">
                  ${t.solar?F`<div class="tile"><div class="th">${_t("wb_sunny",14,"var(--g-amber)")}Solar</div><div class="t-num tvv">${this._num(t.solar)??"—"}°</div></div>`:Y}
                  ${t.power_sensor?F`<div class="tile"><div class="th">${_t("bolt",14,"var(--g-purple)")}Power</div><div class="t-num tvv">${this._num(t.power_sensor)??"—"} <span class="u">kW</span></div></div>`:Y}
                </div>`:Y}
          </div>
        </div>

        ${t.solar_mode||this._modes.length?F`<div class="modes">
              ${t.solar_mode?F`<button class="mode ${g?"on":""}" @click=${e=>this._toggle(t.solar_mode,e)}>${_t("wb_sunny",18,g?"var(--g-amber-ink)":"var(--g-amber)")}Solar</button>`:Y}
              ${this._modes.map(t=>{const e=this._isOn(t.entity),i=t.name??this.hass.states[t.entity]?.attributes.friendly_name??t.entity;return F`<button class="mode ${e?"on":""}" @click=${e=>this._toggle(t.entity,e)}>${_t(t.icon??"bolt",18,e?"var(--g-amber-ink)":"var(--g-dim)")}${i}</button>`})}
            </div>`:Y}
      </div>
    `}};se.styles=[wt,o`
    ...
    `:yt("Add entities to toggle","grid_view"):Y}};re.styles=[wt,o`
      .card { display: flex; flex-direction: column; gap: 14px; }
    ...
    `],e([mt({attribute:!1})],re.prototype,"hass",void 0),e([ft()],re.prototype,"_config",void 0),re=e([pt("glass-toggle-grid-card")],re);const ne={heat:{icon:"local_fire_department",color:"var(--g-amber)",label:"Heat"},cool:{icon:"ac_unit",color:"var(--g-cyan)",label:"Cool"},auto:{icon:"autorenew",color:"var(--g-green)",label:"Auto"},heat_cool:{icon:"device_thermostat",color:"var(--g-green)",label:"Auto"},off:{icon:"power_settings_new",color:"var(--g-dim)",label:"Off"}};let oe=class extends dt{setConfig(t){this._config=t}getCardSize(){return 3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{entity:"",name:"Heat Pump",subtitle:"Pool heater"}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||e.states[this._config.entity]!==this.hass.states[this._config.entity]}get _st(){return this._config.entity?this.hass.states[this._config.entity]:void 0}_step(t,e){e.stopPropagation();const i=this._st.attributes,s=Number(i.target_temp_step??.5),a=Number(i.temperature);if(Number.isNaN(a))return;const r=Number(i.min_temp??15),n=Number(i.max_temp??40);this.hass.callService("climate","set_temperature",{entity_id:this._config.entity,temperature:Math.min(n,Math.max(r,a+t*s))})}_setMode(t){this.hass.callService("climate","set_hvac_mode",{entity_id:this._config.entity,hvac_mode:t})}_toggle(t){t.stopPropagation();if("off"!==this._st.state)this._setMode("off");else{const t=this._st.attributes.hvac_modes??[];this._setMode(t.find(t=>"off"!==t)??"heat")}}render(){if(!this._config||!this.hass)return Y;const t=this._st;if(!t)return yt("Select a heat-pump (climate) entity","heat_pump");const e=t.attributes,i=t.state,s="off"!==i,a=ne[i]??ne.off,r=this._config.name??e.friendly_name??"Heat Pump",n=null!=e.temperature?Number(e.temperature):null,o=Number(e.min_temp??15),l=Number(e.max_temp??40),c=null!=n?Math.max(0,Math.min(100,(n-o)/(l-o)*100)):0,d=(e.hvac_action??i).replace(/_/g," "),g=(e.hvac_modes??[]).filter(t=>["heat","cool","auto","heat_cool"].includes(t));return F`
      <div class="card">
        ...
        ${g.length?F`<div class="seg">
              ${g.map(t=>{const e=ne[t]??ne.off;return F`<button class="sg ${i===t?"on":""}" @click=${()=>this._setMode(t)}>${_t(e.icon,17)}${e.label}</button>`})}
            </div>`:Y}
      </div>
    `}};oe.styles=[wt,o`
      .card { display: flex; flex-direction: column; gap: 18px; }
    ...
    `],e([mt({attribute:!1})],oe.prototype,"hass",void 0),e([ft()],oe.prototype,"_config",void 0),oe=e([pt("glass-heatpump-card")],oe);let le=class extends dt{constructor(){super(...arguments),this._metrics=[]}setConfig(t){this._metrics=(t.metrics??[]).map(t=>"string"==typeof t?{entity:t}:t),this._config={title:"Water Chemistry",...t}}getCardSize(){return this._metrics.length+1}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{title:"Water Chemistry",metrics:[]}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!e||!this.hass||this._metrics.some(t=>e.states[t.entity]!==this.hass.states[t.entity])}_row(t){const e=this.hass.states[t.entity];if(!e)return F`<div class="row"><div class="miss">${t.entity} not found</div></div>`;const i=Number(e.state),s=!Number.isNaN(i),a=t.unit??e.attributes.unit_of_measurement??"",r=t.label??e.attributes.friendly_name??t.entity,n=t.color??"var(--g-cyan)",o=t.min??0,l=t.max??(s?Math.max(2*i,1):100),c=s?Math.max(0,Math.min(100,(i-o)/(l-o)*100)):0;let d=null;null==t.ok_min&&null==t.ok_max||(d=s&&(null==t.ok_min||i>=t.ok_min)&&(null==t.ok_max||i<=t.ok_max)?{label:"OK",cls:"green"}:s&&null!=t.ok_min&&i<t.ok_min?{label:"Low",cls:"amber"}:{label:"High",cls:"red"});const g=null!=t.ok_min?Math.max(0,Math.min(100,(t.ok_min-o)/(l-o)*100)):0,p=null!=t.ok_max?Math.max(0,Math.min(100,(t.ok_max-o)/(l-o)*100)):100;return F`
      <div class="row" @click=${()=>xt(this,"hass-more-info",{entityId:t.entity})}>
        ...
        <div class="body">
          <div class="top">
            <div class="lhs"><span class="lbl">${r}</span>${d?F`<span class="badge ${d.cls}">${d.label}</span>`:Y}</div>
    ...
    `}render(){return this._config&&this.hass?this._metrics.length?F`
      <div class="card">
    ...
    `:yt("Add water metrics (pH, chlorine, …)","science"):Y}};le.styles=[wt,o`
      .card { display: flex; flex-direction: column; gap: 16px; }
    ...
    `],e([mt({attribute:!1})],ce.prototype,"hass",void 0),e([ft()],ce.prototype,"_config",void 0),ce=e([pt("glass-nav-card")],ce);let de=class extends dt{setConfig(t){this._config={variant:"full",...t}}getCardSize(){return"compact"===this._config?.variant?1:4}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{entity:"",variant:"full"}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");if(!e||!this.hass||!this._config)return!0;return[this._stateId,this._config.trigger].filter(Boolean).some(t=>e.states[t]!==this.hass.states[t])}get _stateId(){return this._config.state_entity||this._config.entity}get _st(){return this._stateId?this.hass.states[this._stateId]:void 0}get _hasControl(){const t=this._config;return!!(t.trigger||t.entity&&t.entity.startsWith("cover."))}_openPct(){const t=this._st,e=t.attributes.current_position;return null==e||Number.isNaN(Number(e))?"open"===t.state||"on"===t.state?100:"opening"===t.state?60:"closing"===t.state?40:0:Number(e)}_look(){const t=this._st.state;return"open"===t||"on"===t?{color:"var(--g-amber)",label:"Open",icon:"garage_home"}:"opening"===t||"closing"===t?{color:"var(--g-amber)",label:"opening"===t?"Opening…":"Closing…",icon:"garage_home",moving:!0}:{color:"var(--g-green)",label:"Closed",icon:"garage"}}_toggle(t){if(t.stopPropagation(),!this._hasControl)return;const e=this._config;if(e.trigger){const t=e.trigger.split(".")[0];"button"===t?this.hass.callService("button","press",{entity_id:e.trigger}):"script"===t?this.hass.callService("script","turn_on",{entity_id:e.trigger}):this.hass.callService("homeassistant","toggle",{entity_id:e.trigger})}else if(e.entity&&e.entity.startsWith("cover.")){const t=this._st.state,i="open"===t||"opening"===t?"close_cover":"open_cover";this.hass.callService("cover",i,{entity_id:e.entity})}}render(){if(!this._config||!this.hass)return Y;if(!this._st)return yt("Select a garage cover — or a state sensor + trigger","garage");const t=this._look(),e=this._openPct(),i=this._config.name??this._st.attributes.friendly_name??"Garage",s=this._hasControl;if("compact"===this._config.variant)return F`
      ...
      <div class="card">
        ...
        ${s?F`<button class="btn ${"var(--g-amber)"===t.color?"primary":"soft"}" @click=${this._toggle}>
              ${_t(t.moving?"autorenew":a?"arrow_downward":"arrow_upward",20)}${a?"Close":"Open"}
            </button>`:F`<div class="viewonly">${_t("visibility",18,"var(--g-dim)")}View only — add a trigger to control</div>`}
      </div>
    `}};de.styles=[wt,o`
      @keyframes g-spin { to { transform: rotate(360deg); } }
      .card { display: flex; flex-direction: column; gap: 18px; }
      ...
      <div class="card">
        ...
        <div class="scene">
          <div class="spray" style="opacity:${t?1:0}">
            ${[8,24,42,58,74,90].map((t,e)=>F`<span class="drop" style="left:${t}%;top:${[34,18,8,8,18,34][e]}px;animation-delay:${[0,.2,.45,.3,.15,.5][e]}s"></span>`)}
          </div>
          ...
          <div class="grass"></div>
          <div class="blades">
            ${[16,22,14,20,17].map((e,i)=>F`<span style="height:${e}px;background:${i%2?"#256e3d":"#1f6b38"};${t?`animation:g-sway 1.4s ease-in-out infinite;animation-delay:${.15*i}s`:""}"></span>`)}
          </div>
        </div>
    ...
    `],e([mt({attribute:!1})],ge.prototype,"hass",void 0),e([ft()],ge.prototype,"_config",void 0),ge=e([pt("glass-sprinkler-card")],ge);let pe=class extends dt{constructor(){super(...arguments),this._zones=[]}setConfig(t){this._zones=(t.zones??[]).map(t=>"string"==typeof t?{entity:t}:t),this._config={title:"Irrigation Zones",...t}}getCardSize(){return this._zones.length+1}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{title:"Irrigation Zones",zones:[]}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!e||!this.hass||this._zones.some(t=>e.states[t.entity]!==this.hass.states[t.entity])}_on(t){const e=this.hass.states[t]?.state;return"on"===e||"open"===e}_toggle(t){this.hass.callService("homeassistant","toggle",{entity_id:t})}render(){if(!this._config||!this.hass)return Y;if(!this._zones.length)return yt("Add irrigation zones (switches/valves)","water_drop");const t=this._zones.filter(t=>this._on(t.entity)).length;return F`
      <div class="card">
        ...
        <div class="zones">
          ${this._zones.map(t=>{const e=this.hass.states[t.entity];if(!e)return F`<div class="zone"><div class="miss">${t.entity} not found</div></div>`;const i=this._on(t.entity),s=t.name??e.attributes.friendly_name??t.entity,a=i?"var(--g-cyan)":"var(--g-dim)";return F`
              <button class="zone ${i?"on":""}" @click=${()=>this._toggle(t.entity)}>
                <div class="well">
                  ${i?F`<span class="ripple"></span>`:Y}
    ...
    `}};pe.styles=[wt,o`
      ...
      .miss { color: var(--g-red-text); font-size: 12px; }
    `],e([mt({attribute:!1})],pe.prototype,"hass",void 0),e([ft()],pe.prototype,"_config",void 0),pe=e([pt("glass-irrigation-card")],pe);const he={weather:"partly_cloudy_day",alarm_control_panel:"verified_user",light:"lightbulb",lock:"lock",climate:"thermostat",binary_sensor:"sensors",person:"person"},ue={amber:"var(--g-amber)",green:"var(--g-green)",cyan:"var(--g-cyan)",purple:"var(--g-purple)",red:"var(--g-red-text)",dim:"var(--g-dim)"};let me=class extends dt{constructor(){super(...arguments),this._badges=[]}setConfig(t){this._badges=(t.badges??[]).map(t=>"string"==typeof t?{entity:t}:t),this._config={variant:"strip",...t}}getCardSize(){return 1}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{variant:"strip",badges:[]}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!e||!this.hass||this._badges.some(t=>e.states[t.entity]!==this.hass.states[t.entity])}_value(t){const e=this.hass.states[t];if(!e)return"—";const i=t.split(".")[0],s=e.attributes.device_class;if("weather"===i){const t=e.attributes.temperature;return null!=t?`${Math.round(Number(t))}°`:e.state}if("alarm_control_panel"===i)return"disarmed"===e.state?"Disarmed":"Armed";if("binary_sensor"===i||"switch"===i||"light"===i||"input_boolean"===i)return"door"===s||"window"===s||"opening"===s?"on"===e.state?"Open":"Closed":"on"===e.state?"On":"Off";const a=e.attributes.unit_of_measurement,r=Number(e.state),n=Number.isNaN(r)?e.state.replace(/_/g," ").replace(/^\w/,t=>t.toUpperCase()):Math.abs(r)>=100?Math.round(r):r;return a?`${n}${a.startsWith("°")?"":" "}${a}`:String(n)}_sublabel(t){const e=this.hass.states[t.entity];return t.label??e?.attributes.friendly_name??t.entity}_iconFor(t){if(t.icon)return t.icon;const e=this.hass.states[t.entity],i=e?.attributes.device_class;return"temperature"===i?"thermostat":"humidity"===i?"water_drop":"battery"===i?"battery_full":"power"===i||"energy"===i?"bolt":he[t.entity.split(".")[0]]??"info"}_color(t){if(t.color)return ue[t.color]??t.color;const e=this.hass.states[t.entity];return e&&("on"===e.state||"home"===e.state||e.state?.startsWith("armed"))?"var(--g-green)":"var(--g-amber)"}_open(t){xt(this,"hass-more-info",{entityId:t})}render(){if(!this._config||!this.hass)return Y;if(!this._badges.length)return yt("Add status badges","badge");return"circular"===this._config.variant?F`
        <div class="circ-wrap">
          ${this._badges.map(t=>F`
              <button class="circ" @click=${()=>this._open(t.entity)}>
                <div class="circ-ic">${_t(this._iconFor(t),26,this._color(t))}</div>
                <span class="circ-l">${this._value(t.entity)}</span>
              </button>
            `)}
        </div>
      `:F`
      <div class="strip">
        ${this._badges.map(t=>F`
            <button class="badge" @click=${()=>this._open(t.entity)}>
              ${_t(this._iconFor(t),20,this._color(t))}
              <div class="txt"><div class="v">${this._value(t.entity)}</div><div class="s">${this._sublabel(t)}</div></div>
            </button>
          `)}
      </div>
    `}};me.styles=[wt,o`
    ...
    `],e([mt({attribute:!1})],me.prototype,"hass",void 0),e([ft()],me.prototype,"_config",void 0),me=e([pt("glass-badge-strip-card")],me);let fe=class extends dt{setConfig(t){this._config=t}getCardSize(){return 1}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{entity:""}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||[this._config.entity,this._config.battery].filter(Boolean).some(t=>e.states[t]!==this.hass.states[t])}_batteryIcon(t){return t>=95?"battery_full":t>=60?"battery_5_bar":t>=40?"battery_3_bar":t>=15?"battery_2_bar":"battery_alert"}render(){if(!this._config||!this.hass)return Y;const t=this._config.entity?this.hass.states[this._config.entity]:void 0;if(!t)return yt("Select a person entity","person");const e="home"===t.state,i=e?"Home":"not_home"===t.state?"Away":t.state.charAt(0).toUpperCase()+t.state.slice(1),s=e?"var(--g-green)":"not_home"===t.state?"var(--g-dim)":"var(--g-amber)",a=this._config.name??t.attributes.friendly_name??this._config.entity,r=t.attributes.entity_picture,n=a.charAt(0).toUpperCase(),o=e?"home":"not_home"===t.state?"directions_walk":"location_on",l=this._config.battery?Number(this.hass.states[this._config.battery]?.state):NaN,c=!Number.isNaN(l);return F`
      <button class="card" @click=${()=>xt(this,"hass-more-info",{entityId:this._config.entity})}>
        <div class="av-wrap">
          ${r?F`<img class="av" src=${r} alt=${a} />`:F`<div class="av mono">${n}</div>`}
          <span class="dot" style="background:${e?"var(--g-green)":"var(--g-dim)"}"></span>
        </div>
        <div class="mid">
          <div class="name">${a}</div>
          <div class="zone" style="color:${s}">${_t("location_on",15,s)}${i}</div>
        </div>
        <div class="right">
          ${c?F`<div class="batt">${_t(this._batteryIcon(l),16,"var(--g-text)")}${Math.round(l)}</div>`:Y}
          ${_t(o,17,e?"var(--g-green)":"var(--g-dim)")}
        </div>
      </button>
    `}};fe.styles=[wt,o`
      .card {
    ...
    `],e([mt({attribute:!1})],fe.prototype,"hass",void 0),e([ft()],fe.prototype,"_config",void 0),fe=e([pt("glass-person-badge-card")],fe);const ve={"clear-night":{icon:"bedtime",label:"Clear night"},cloudy:{icon:"cloud",label:"Cloudy"},fog:{icon:"foggy",label:"Fog"},hail:{icon:"weather_hail",label:"Hail"},lightning:{icon:"thunderstorm",label:"Lightning"},"lightning-rainy":{icon:"thunderstorm",label:"Storms"},partlycloudy:{icon:"partly_cloudy_day",label:"Partly cloudy"},pouring:{icon:"rainy",label:"Heavy rain"},rainy:{icon:"rainy",label:"Rain"},snowy:{icon:"weather_snowy",label:"Snow"},"snowy-rainy":{icon:"weather_mix",label:"Sleet"},sunny:{icon:"clear_day",label:"Sunny"},windy:{icon:"air",label:"Windy"},"windy-variant":{icon:"air",label:"Windy"},exceptional:{icon:"warning",label:"Severe"}},be=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],xe=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];let _e=class extends dt{constructor(){super(...arguments),this._now=new Date,this._open=()=>{this._config?.weather&&xt(this,"hass-more-info",{entityId:this._config.weather})}}setConfig(t){this._config=t}getCardSize(){return"compact"===this._config?.layout?2:3}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{weather:"",layout:"full",show_seconds:!0,show_forecast:!0,forecast_days:4}}connectedCallback(){super.connectedCallback(),this._timer=window.setInterval(()=>this._now=new Date,1e3)}disconnectedCallback(){super.disconnectedCallback(),this._timer&&window.clearInterval(this._timer)}_greeting(t){return t<12?"Good morning":t<18?"Good afternoon":"Good evening"}_forecast(){const t=this._config?.weather?this.hass?.states[this._config.weather]:void 0,e=t?.attributes.forecast;if(!Array.isArray(e))return[];const i=this._config?.forecast_days??4;return e.slice(0,i)}render(){if(!this._config||!this.hass)return Y;const t="compact"===this._config.layout,e=this._now,i=`${String(e.getHours()).padStart(2,"0")}:${String(e.getMinutes()).padStart(2,"0")}`,s=String(e.getSeconds()).padStart(2,"0"),a=`${be[e.getDay()]}, ${e.getDate()} ${xe[e.getMonth()]}`,r=this._greeting(e.getHours()),n=this._config.weather?this.hass.states[this._config.weather]:void 0;if(this._config.weather&&!n)return yt("Select a weather entity","schedule");let o,l="",c="";if(n){o=ve[n.state]??{icon:"thermostat",label:n.state};const t=n.attributes;c=t.temperature_unit||`°${this.hass.config.unit_system.temperature.replace("°","")}`,l=null!=t.temperature?String(Math.round(Number(t.temperature))):"—"}if(t)return F`
        <div class="card compact" @click=${this._open}>
          <div class="c-clock">
            <div class="c-hhmm">${i}</div>
            <div class="c-date">${a}</div>
          </div>
          ${n?F`<div class="c-wx">
                ${_t(o.icon,38,"var(--g-amber)")}
                <div class="c-wx-txt">
                  <div class="c-temp">${l}°</div>
                  <div class="c-cond">${o.label}</div>
                </div>
              </div>`:Y}
        </div>
      `;const d=!1!==this._config.show_forecast?this._forecast():[];return F`
      <div class="card full" @click=${this._open}>
        <div class="clock">
          <div class="greeting">${r}</div>
          <div class="hhmm-row">
            <span class="hhmm">${i}</span>
            ${!1!==this._config.show_seconds?F`<span class="secs">${s}</span>`:Y}
          </div>
          <div class="date">${a}</div>
        </div>

        ${n?F`<div class="wx">
              ${_t(o.icon,52,"var(--g-amber)")}
              <div class="wx-temp"><span class="wx-num">${l}</span><span class="wx-unit">${c}</span></div>
              <div class="wx-cond">${o.label}</div>
            </div>`:Y}

        ${d.length?F`<div class="fc">
              ${d.map(t=>{const e=new Date(t.datetime),i=be[e.getDay()].slice(0,3),s=t.condition?ve[t.condition]:void 0;return F`<div class="fc-day">
                  <span class="fc-lbl">${i}</span>
                  ${_t(s?.icon??"thermostat",24,"var(--g-text)")}
                  <div class="fc-hi">${null!=t.temperature?Math.round(Number(t.temperature)):"—"}°</div>
                  <div class="fc-lo">${null!=t.templow?`${Math.round(Number(t.templow))}°`:""}</div>
                </div>`})}
            </div>`:Y}
      </div>
    `}};_e.styles=[wt,o`
    ...
    `],e([mt({attribute:!1})],_e.prototype,"hass",void 0),e([ft()],_e.prototype,"_config",void 0),e([ft()],_e.prototype,"_now",void 0),_e=e([pt("glass-clock-weather-card")],_e);const ye={amber:"var(--g-amber)",cyan:"var(--g-cyan)",purple:"var(--g-purple)",green:"var(--g-green)",red:"var(--g-red-text)"};let we=class extends dt{constructor(){super(...arguments),this._toggle=t=>{t.stopPropagation();const e=this._config?.lights??[];if(!e.length||!this.hass)return;const i=this._lightsOn().length>0;this.hass.callService("light",i?"turn_off":"turn_on",{entity_id:e})}}setConfig(t){this._config={name:"Room",...t}}getCardSize(){return 2}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{name:"Living Room",icon:"weekend",color:"amber",lights:[]}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");if(!e||!this.hass||!this._config)return!0;const i=this._config;return[...i.lights??[],i.temp,i.humidity,i.devices].filter(Boolean).some(t=>e.states[t]!==this.hass.states[t])}_num(t){if(!t)return"—";const e=this.hass?.states[t];if(!e)return"—";const i=Number(e.state);return Number.isNaN(i)?"—":String(Math.round(10*i)/10)}_lightsOn(){return(this._config?.lights??[]).filter(t=>"on"===this.hass?.states[t]?.state)}render(){if(!this._config||!this.hass)return Y;if(!this._config.name&&!this._config.lights?.length&&!this._config.temp)return yt("Configure this room","meeting_room");const t=ye[this._config.color??"amber"]??"var(--g-amber)",e=this._config.lights??[],i=this._lightsOn().length,s=i>0,a=e.length?s?`${i} of ${e.length} lights on`:"All off":"No lights";let r="—";if(this._config.devices){const t=this._num(this._config.devices);r="—"!==t?t:String(this.hass.states[this._config.devices]?1:0)}else e.length&&(r=String(e.length));return F`
      <div class="card ${s?"on":""}" style="--accent:${t}">
        <div class="head">
          <div class="well">${_t(this._config.icon??"meeting_room",28,s?"var(--accent)":"var(--g-dim)")}</div>
          <div class="who">
            <div class="name">${this._config.name}</div>
            <div class="status" style="color:${s?t:"var(--g-dim)"}">${a}</div>
          </div>
          ${e.length?F`<div class="track ${s?"on":""}" @click=${this._toggle} role="switch" aria-checked=${s}>
                <div class="knob"></div>
              </div>`:Y}
        </div>
        ...
        <div class="tiles">
          <div class="tile">
            <div class="tile-h">${_t("device_thermostat",15,"var(--g-amber)")}Temp</div>
            <div class="tile-v">${this._num(this._config.temp)}°</div>
          </div>
          <div class="tile">
            <div class="tile-h">${_t("water_drop",15,"var(--g-cyan)")}Humid</div>
            <div class="tile-v">${this._num(this._config.humidity)}%</div>
          </div>
          <div class="tile">
            <div class="tile-h">${_t("devices_other",15,"var(--g-purple)")}Devices</div>
            <div class="tile-v">${r}</div>
          </div>
        </div>
      </div>
    `}};we.styles=[wt,o`
    ...
    `],e([mt({attribute:!1})],we.prototype,"hass",void 0),e([ft()],we.prototype,"_config",void 0),we=e([pt("glass-room-card")],we);const $e=[["#F3D06A","#e0b24a","#221a02"],["#c9a6ff","#a97ff0","#1c1030"],["#87dde1","#4fb9bf","#052423"],["#B9F6A6","#7fd98a","#0f1a0d"],["#ff8080","#e05a5a","#2a0808"]];let ke=class extends dt{setConfig(t){this._config={entities:[],...t}}getCardSize(){return 1}static getConfigElement(){return document.createElement("glass-config-editor")}static getStubConfig(){return{entities:[]}}shouldUpdate(t){if(t.has("_config"))return!0;const e=t.get("hass");return!(e&&this.hass&&this._config)||this._config.entities.some(t=>e.states[t]!==this.hass.states[t])}render(){if(!this._config||!this.hass)return Y;if(!this._config.entities.length)return yt("Add person entities","group");const t=this._config.entities.map(t=>({id:t,st:this.hass.states[t]})),e=t.filter(t=>"home"===t.st?.state).length,i=e===t.length,s=0===e?"Everyone away":1===e?"1 person home":`${e} people home`;return F`
      <div class="card">
        <div class="avatars">
          ${t.map((e,i)=>{const[s,a,r]=$e[i%$e.length],n=e.st?.attributes.entity_picture,o=e.st?.attributes.friendly_name??e.id,l=o.charAt(0).toUpperCase(),c="home"===e.st?.state;return F`<button
              class="av"
              title=${o}
              @click=${()=>xt(this,"hass-more-info",{entityId:e.id})}
              style="--g1:${s};--g2:${a};--fg:${r};z-index:${t.length-i};margin-left:${i?"-8px":"0"};opacity:${c?1:.55}"
            >
              ${n?F`<img src=${n} alt=${o} />`:F`<span>${l}</span>`}
            </button>`})}
        </div>
        <span class="label">${s}</span>
        <span class="badge ${i?"green":e?"amber":""}">
          <span class="dot"></span>${e?"Home":"Away"}
        </span>
      </div>
    `}};ke.styles=[wt,o`
    ...
    `],e([mt({attribute:!1})],ke.prototype,"hass",void 0),e([ft()],ke.prototype,"_config",void 0),ke=e([pt("glass-people-row-card")],ke);const Se=(t,e=!1)=>({entity:{...t?{domain:t}:{},...e?{multiple:!0}:{}}}),Ce={text:{}},ze=(t,e)=>({number:{min:t,max:e,mode:"box"}}),Ae=t=>({select:{mode:"dropdown",options:t}}),Ee={boolean:{}},Ne={"glass-person-card":[{name:"title",selector:Ce},{name:"entities",required:!0,selector:Se("person",!0)}],"glass-weather-card":[{name:"entity",required:!0,selector:Se("weather")},{name:"sun",selector:Se("sun")},{name:"humidity",selector:Se("sensor")}],"glass-camera-card":[{name:"entity",required:!0,selector:Se("camera")},{name:"name",selector:Ce},{name:"icon",selector:Ce},{name:"show_timestamp",selector:Ee}],"glass-alarm-card":[{name:"entity",required:!0,selector:Se("alarm_control_panel")},{name:"variant",selector:Ae([{value:"shield",label:"Shield (default)"},{value:"radial",label:"Radial"},{value:"bar",label:"Compact bar"},{value:"keypad",label:"Keypad"},{value:"triggered",label:"Triggered / status"}])},{name:"name",selector:Ce},{name:"subtitle",selector:Ce},{name:"code",selector:Ce},{name:"code_length",selector:ze(1,8)},{name:"buttons",selector:{select:{multiple:!0,mode:"list",options:[{value:"disarm",label:"Disarm"},{value:"arm",label:"Arm (single button)"},{value:"arm_home",label:"Arm Home"},{value:"arm_away",label:"Arm Away"},{value:"arm_night",label:"Arm Night"},{value:"arm_vacation",label:"Arm Vacation"}]}}}],"glass-media-card":[{name:"entity",required:!0,selector:Se("media_player")},{name:"name",selector:Ce}],"glass-flight-card":[{name:"entity",required:!0,selector:Se("sensor")},{name:"title",selector:Ce},{name:"max",selector:ze(1,10)}],"glass-tile-card":[{name:"entity",required:!0,selector:Se()},{name:"name",selector:Ce},{name:"icon",selector:Ce}],"glass-energy-card":[{name:"title",selector:Ce},{name:"variant",selector:Ae([{value:"flow",label:"Power flow (default)"},{value:"ring",label:"Battery ring"},{value:"stats",label:"Stat trio"},{value:"meters",label:"Meters"},{value:"production",label:"Production bars"}])},{name:"solar",selector:Se("sensor")},{name:"grid",selector:Se("sensor")},{name:"battery",selector:Se("sensor")},{name:"battery_soc",selector:Se("sensor")},{name:"house",selector:Se("sensor")},{name:"today",selector:Se("sensor")},{name:"imported",selector:Se("sensor")},{name:"exported",selector:Se("sensor")},{name:"saved",selector:Se("sensor")},{name:"meters",selector:Se("sensor",!0)},{name:"production",selector:Se("sensor")}],"glass-sensor-list-card":[{name:"title",selector:Ce},{name:"entities",required:!0,selector:Se(["cover","binary_sensor"],!0)}],"glass-appliance-card":[{name:"name",selector:Ce},{name:"subtitle",selector:Ce},{name:"icon",selector:Ce},{name:"icon_color",selector:{select:{mode:"dropdown",options:["cyan","amber","green","purple","red"]}}},{name:"status",selector:Se(["sensor","binary_sensor"])},{name:"remaining",selector:Se("sensor")},{name:"total",selector:Se("sensor")},{name:"toggle",selector:Se(["switch","input_boolean"])},{name:"stats",selector:Se(["sensor","binary_sensor","number"],!0)}],"glass-fridge-card":[{name:"name",selector:Ce},{name:"subtitle",selector:Ce},{name:"door",selector:Se("binary_sensor")},{name:"wifi",selector:Se("binary_sensor")},{name:"fridge_temp",selector:Se("number")},{name:"freezer_temp",selector:Se("number")},{name:"toggles",selector:Se(["switch","input_boolean"],!0)}],"glass-pool-card":[{name:"name",selector:Ce},{name:"subtitle",selector:Ce},{name:"switch",selector:Se(["switch","input_boolean"])},{name:"energy",selector:Se("sensor")},{name:"color",selector:Ae(["green","amber","cyan","purple","red"])}],"glass-stat-card":[{name:"entity",required:!0,selector:Se()},{name:"name",selector:Ce},{name:"icon",selector:Ce},{name:"color",selector:Ae(["amber","green","cyan","purple","red","dim"])}],"glass-lights-card":[{name:"title",selector:Ce},{name:"entities",required:!0,selector:Se("light",!0)}],"glass-scenes-card":[{name:"title",selector:Ce},{name:"columns",selector:ze(2,6)},{name:"scenes",required:!0,selector:Se(["scene","script","automation"],!0)}],"glass-toggle-grid-card":[{name:"title",selector:Ce},{name:"columns",selector:ze(2,8)},{name:"entities",required:!0,selector:Se(["light","switch","fan","input_boolean","script","scene"],!0)}],"glass-heatpump-card":[{name:"entity",required:!0,selector:Se("climate")},{name:"name",selector:Ce},{name:"subtitle",selector:Ce}],"glass-garage-card":[{name:"entity",selector:Se("cover")},{name:"state_entity",selector:Se(["binary_sensor","cover"])},{name:"trigger",selector:Se(["switch","button","script","input_boolean"])},{name:"variant",selector:Ae([{value:"full",label:"Full (animated visual)"},{value:"compact",label:"Compact tile"}])},{name:"name",selector:Ce},{name:"subtitle",selector:Ce}],"glass-badge-strip-card":[{name:"variant",selector:Ae([{value:"strip",label:"Strip"},{value:"circular",label:"Circular"}])},{name:"badges",required:!0,selector:Se(void 0,!0)}],"glass-person-badge-card":[{name:"entity",required:!0,selector:Se(["person","device_tracker"])},{name:"name",selector:Ce},{name:"battery",selector:Se("sensor")}],"glass-sprinkler-card":[{name:"entity",required:!0,selector:Se(["switch","valve","input_boolean"])},{name:"variant",selector:Ae([{value:"hero",label:"Hero (animated)"},{value:"compact",label:"Compact tile"}])},{name:"name",selector:Ce},{name:"subtitle",selector:Ce}],"glass-irrigation-card":[{name:"title",selector:Ce},{name:"zones",required:!0,selector:Se(["switch","valve","input_boolean"],!0)}],"glass-water-chemistry-card":[{name:"title",selector:Ce},{name:"metrics",selector:Se("sensor",!0)}],"glass-nav-card":[{name:"variant",selector:Ae([{value:"dock",label:"Full-width dock"},{value:"pill",label:"Floating pill"}])},{name:"fixed",selector:Ee},{name:"max_width",selector:ze(320,1600)}],"glass-light-card":[{name:"entity",required:!0,selector:Se("light")},{name:"name",selector:Ce},{name:"subtitle",selector:Ce}],"glass-aircon-card":[{name:"entity",required:!0,selector:Se("climate")},{name:"name",selector:Ce},{name:"subtitle",selector:Ce}],"glass-geyser-card":[{name:"name",selector:Ce},{name:"subtitle",selector:Ce},{name:"power",selector:Se(["switch","input_boolean","water_heater"])},{name:"current",selector:Se("sensor")},{name:"target",selector:Se(["number","water_heater","climate"])},{name:"min_temp",selector:ze(0,100)},{name:"max_temp",selector:ze(0,100)},{name:"solar",selector:Se("sensor")},{name:"power_sensor",selector:Se("sensor")},{name:"solar_mode",selector:Se(["switch","input_boolean"])},{name:"modes",selector:Se(["switch","input_boolean"],!0)}],"glass-clock-weather-card":[{name:"weather",selector:Se("weather")},{name:"layout",selector:Ae([{value:"full",label:"Full (desktop hero)"},{value:"compact",label:"Compact (mobile strip)"}])},{name:"show_seconds",selector:Ee},{name:"show_forecast",selector:Ee},{name:"forecast_days",selector:ze(1,7)}],"glass-room-card":[{name:"name",required:!0,selector:Ce},{name:"icon",selector:Ce},{name:"color",selector:Ae(["amber","cyan","purple","green","red"])},{name:"lights",selector:Se("light",!0)},{name:"temp",selector:Se("sensor")},{name:"humidity",selector:Se("sensor")},{name:"devices",selector:Se("sensor")}],"glass-people-row-card":[{name:"entities",required:!0,selector:Se(["person","device_tracker"],!0)}],"glass-dishwasher-card":[{name:"name",selector:Ce},{name:"subtitle",selector:Ce},{name:"status",selector:Se("sensor")},{name:"alert",selector:Se("binary_sensor")},{name:"alert_text",selector:Ce},{name:"tiles",selector:Se(["binary_sensor","sensor"],!0)},{name:"levels",selector:Se("sensor",!0)},{name:"level_max",selector:ze(1,10)}]},Me={entity:"Entity",entities:"Entities",title:"Title",name:"Name (optional)",icon:"Icon (optional)",sun:"Sun entity (for sunrise/sunset)",humidity:"Humidity sensor (optional)",code:"Arm/disarm code (optional)",max:"Max flights to show",buttons:"Action buttons to show",variant:"Layout / style",code_length:"Keypad code length",today:"Solar-today energy sensor (big number)",imported:"Imported-today sensor",exported:"Exported-today sensor",saved:"Money-saved sensor",meters:"Percentage sensors to show as meters",production:"Production sensor for hourly bars (needs statistics)",subtitle:"Subtitle (e.g. room/area)",icon_color:"Icon accent colour",status:"Status sensor (drives the pill)",remaining:"Remaining-time sensor (timestamp → progress ring)",total:"Total-time sensor (minutes, for the ring %)",toggle:"Power switch (optional on/off button)",stats:"Extra sensors shown as stat tiles",door:"Door sensor",wifi:"Wi-Fi / connectivity sensor (optional)",fridge_temp:"Fridge temperature (number entity)",freezer_temp:"Freezer temperature (number entity)",toggles:"Feature switches (Express cool/mode, etc.)",switch:"Pump switch",energy:"Energy sensor (kWh)",color:"Accent colour",columns:"Columns",scenes:"Scenes / scripts to show",alert:"Alert sensor (e.g. refill needed)",alert_text:"Alert message",tiles:"Status tiles",levels:"Level sensors (shown as bars)",level_max:"Level bar maximum",power:"Power switch (on/off)",current:"Current temperature sensor",target:"Target temperature (number / thermostat)",min_temp:"Minimum temperature",max_temp:"Maximum temperature",solar:"Solar collector temperature sensor",power_sensor:"Power draw sensor (kW)",solar_mode:"Solar mode switch (toggleable)",modes:"Extra mode switches (boost, element, timer)",metrics:"Water metric sensors (pH, chlorine, …)",fixed:"Stick to bottom of screen",max_width:"Maximum width (px)",show_timestamp:"Show date/time overlay",state_entity:"State sensor (open / closed)",trigger:"Trigger to open/close (switch / button / script)",badges:"Status badges",battery:"Battery sensor (optional)",solar:"Solar power sensor",grid:"Grid power sensor (+import / −export)",battery:"Battery power sensor (+discharge / −charge)",battery_soc:"Battery charge % sensor",house:"House load sensor (optional; derived if empty)",weather:"Weather entity (optional — omit for clock only)",layout:"Layout",show_seconds:"Show seconds",show_forecast:"Show forecast",forecast_days:"Forecast days",lights:"Room lights (master toggle)",temp:"Temperature sensor",devices:"Device-count sensor (optional)"},Pe={icon:"Material Symbols name, e.g. lightbulb, garage, kitchen",code:"Only if your panel requires a code to arm/disarm"};let Oe=class extends dt{constructor(){super(...arguments),this._label=t=>Me[t.name]??t.name,this._helper=t=>Pe[t.name]??""}setConfig(t){this._config=t}_valueChanged(t){if(t.stopPropagation(),!this._config)return;const e={...this._config,...t.detail.value,type:this._config.type};xt(this,"config-changed",{config:e})}render(){if(!this.hass||!this._config)return Y;const t=this._config.type.replace(/^custom:/,""),e=Ne[t];return e?F`
      <ha-form
        .hass=${this.hass}
        .data=${this._config}
        .schema=${e}
      ...
      ></ha-form>
    `:F`<div class="fallback">No visual editor for <code>${this._config.type}</code> — edit in YAML.</div>`}};Oe.styles=o`
    .fallback { padding: 12px; color: var(--secondary-text-color, #8b9099); font-size: 14px; }
    code { font-family: monospace; }
  `,e([mt({attribute:!1})],Oe.prototype,"hass",void 0),e([ft()],Oe.prototype,"_config",void 0),Oe=e([pt("glass-config-editor")],Oe);const je=["light","switch","fan","input_boolean","script","scene","automation","cover"],Le=[{name:"title",selector:{text:{}}},{name:"columns",selector:{number:{min:1,max:12,mode:"box"}}}];let Ue=class extends dt{constructor(){super(...arguments),this._headLabel=t=>"columns"===t.name?"Columns":"Title"}setConfig(t){this._config=t}get _items(){return(this._config?.entities??[]).map(t=>"string"==typeof t?{entity:t}:{...t})}_emit(t){xt(this,"config-changed",{config:{...this._config,...t}})}_headChanged(t){t.stopPropagation(),this._emit(t.detail.value)}_setItems(t){const e=t.map(t=>{const e={entity:t.entity};return t.name&&(e.name=t.name),t.icon&&(e.icon=t.icon),e});this._emit({entities:e})}_update(t,e){const i=this._items;i[t]={...i[t],...e},this._setItems(i)}_remove(t){const e=this._items;e.splice(t,1),this._setItems(e)}_add(){this._setItems([...this._items,{entity:""}])}render(){if(!this.hass||!this._config)return Y;const t=this._items;return F`
      <div class="wrap">
        <ha-form
          .hass=${this.hass}
        ...
        ${t.map((t,e)=>F`
            <div class="item">
              ...
              <div class="item-bot">
                ...
                ></ha-textfield>
                <span class="prev">${t.icon?F`<span class="ms">${t.icon}</span>`:"—"}</span>
                <ha-textfield
                  class="grow"
                  label="Name (optional)"
                  .value=${t.name??""}
